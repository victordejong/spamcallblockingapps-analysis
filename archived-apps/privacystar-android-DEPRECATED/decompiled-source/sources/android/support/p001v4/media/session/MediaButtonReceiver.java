...
public class MediaButtonReceiver extends BroadcastReceiver {
    ...
    /* loaded from: classes-dex2jar.jar:android/support/v4/media/session/MediaButtonReceiver$MediaButtonConnectionCallback.class */
    private static class MediaButtonConnectionCallback extends MediaBrowserCompat.ConnectionCallback {
        ...
        public void onConnected() {
            try {
                new MediaControllerCompat(this.mContext, this.mMediaBrowser.getSessionToken()).dispatchMediaButtonEvent((KeyEvent) this.mIntent.getParcelableExtra(CallBlockingUtil.MediaButtonConsts.extraKeyEvent));
    ...
    public static PendingIntent buildMediaButtonPendingIntent(Context context, ComponentName componentName, long j) {
        ...
        Intent intent = new Intent(CallBlockingUtil.MediaButtonConsts.actionMediaButton);
        intent.setComponent(componentName);
        intent.putExtra(CallBlockingUtil.MediaButtonConsts.extraKeyEvent, new KeyEvent(0, keyCode));
    ...
    public static KeyEvent handleIntent(MediaSessionCompat mediaSessionCompat, Intent intent) {
        if (mediaSessionCompat == null || intent == null || !CallBlockingUtil.MediaButtonConsts.actionMediaButton.equals(intent.getAction()) || !intent.hasExtra(CallBlockingUtil.MediaButtonConsts.extraKeyEvent)) {
            return null;
        }
        KeyEvent keyEvent = (KeyEvent) intent.getParcelableExtra(CallBlockingUtil.MediaButtonConsts.extraKeyEvent);
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent == null || !CallBlockingUtil.MediaButtonConsts.actionMediaButton.equals(intent.getAction()) || !intent.hasExtra(CallBlockingUtil.MediaButtonConsts.extraKeyEvent)) {
            Log.d(TAG, "Ignore unsupported intent: " + intent);
            return;
        }
        ComponentName serviceComponentByAction = getServiceComponentByAction(context, CallBlockingUtil.MediaButtonConsts.actionMediaButton);

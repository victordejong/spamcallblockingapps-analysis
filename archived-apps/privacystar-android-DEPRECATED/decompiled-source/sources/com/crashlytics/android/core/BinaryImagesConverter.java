...
class BinaryImagesConverter {
    ...
    private File correctDataPath(File file) {
        ...
        if (file.getAbsolutePath().startsWith(DATA_DIR)) {
            try {
                ...
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Error getting ApplicationInfo", e);
                file2 = file;
            }
        }
        return file2;
    ...
    private static byte[] generateBinaryImagesJsonString(JSONArray jSONArray) {
        ...
        try {
            ...
            Fabric.getLogger().mo279w(CrashlyticsCore.TAG, "Binary images string is null", e);
            return new byte[0];
        }
    }
    ...
    private JSONObject jsonFromMapEntryString(String str) {
        ...
        try {
            try {
                ...
                Fabric.getLogger().mo287d(CrashlyticsCore.TAG, "Could not create a binary image json string", e);
                return null;
            }
        } catch (IOException e2) {
            Logger logger = Fabric.getLogger();
            logger.mo287d(CrashlyticsCore.TAG, "Could not generate ID for file " + parse.path, e2);
            return null;
        }
    }
    ...
    private JSONArray parseProcMapsJsonFromString(String str) {
        ...
        try {
            ...
            Fabric.getLogger().mo279w(CrashlyticsCore.TAG, "Unable to parse proc maps string", e);
            return jSONArray;
        }
    }

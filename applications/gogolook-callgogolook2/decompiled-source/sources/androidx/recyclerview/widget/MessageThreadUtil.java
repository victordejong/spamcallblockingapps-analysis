...
public class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public class C10591 implements ThreadUtil.MainThreadCallback<T> {
        ...
        public Runnable mMainThreadRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.1.1
            ...
            public void run() {
                SyncQueueItem next = C10591.this.mQueue.next();
                while (next != null) {
                    int i = next.what;
                    if (i == 1) {
                        C10591.this.val$callback.updateItemCount(next.arg1, next.arg2);
                    } else if (i == 2) {
                        C10591.this.val$callback.addTile(next.arg1, (TileList.Tile) next.data);
                    } else if (i != 3) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C10591.this.val$callback.removeTile(next.arg1, next.arg2);
                    }
                    next = C10591.this.mQueue.next();
                }
            }
        };

        public C10591(ThreadUtil.MainThreadCallback mainThreadCallback) {
    ...
    public class C10612 implements ThreadUtil.BackgroundCallback<T> {
        ...
        public Runnable mBackgroundRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.2.1
            ...
            public void run() {
                while (true) {
                    SyncQueueItem next = C10612.this.mQueue.next();
                    if (next == null) {
                        C10612.this.mBackgroundRunning.set(false);
                        return;
                    }
                    int i = next.what;
                    if (i == 1) {
                        C10612.this.mQueue.removeMessages(1);
                        C10612.this.val$callback.refresh(next.arg1);
                    } else if (i == 2) {
                        C10612.this.mQueue.removeMessages(2);
                        C10612.this.mQueue.removeMessages(3);
                        C10612.this.val$callback.updateRange(next.arg1, next.arg2, next.arg3, next.arg4, next.arg5);
                    } else if (i == 3) {
                        C10612.this.val$callback.loadTile(next.arg1, next.arg2);
                    } else if (i != 4) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C10612.this.val$callback.recycleTile((TileList.Tile) next.data);
                    }
                }
            }
        };

        public C10612(ThreadUtil.BackgroundCallback backgroundCallback) {
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new C10612(backgroundCallback);
    }

    @Override // androidx.recyclerview.widget.ThreadUtil
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new C10591(mainThreadCallback);
    }
}

...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f127mX = new float[min];
            this.f128mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
                this.f127mX[i2] = fArr[0];
                this.f128mY[i2] = fArr[1];
            }
            if (Math.abs(this.f127mX[0]) <= 1.0E-5d && Math.abs(this.f128mY[0]) <= 1.0E-5d) {
                int i3 = min - 1;
                if (Math.abs(this.f127mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f128mY[i3] - 1.0f) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        float[] fArr2 = this.f127mX;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
                            i++;
            ...
            sb.append(this.f127mX[0]);
            sb.append(",");
            sb.append(this.f128mY[0]);
            sb.append(" end:");
            int i5 = min - 1;
            sb.append(this.f127mX[i5]);
            sb.append(",");
            sb.append(this.f128mY[i5]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f127mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f127mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f127mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f128mY[i];
        }
        float f3 = (f - fArr[i]) / f2;
        float[] fArr2 = this.f128mY;
        float f4 = fArr2[i];

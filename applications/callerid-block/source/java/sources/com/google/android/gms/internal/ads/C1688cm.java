package com.google.android.gms.internal.ads;

import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.Locale;
import java.util.Map;
import java.util.TimeZone;
/* renamed from: com.google.android.gms.internal.ads.cm */
/* loaded from: classes-dex2jar.jar:com/google/android/gms/internal/ads/cm.class */
public final class C1688cm {
    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r0v106, types: [long] */
    /* JADX WARN: Type inference failed for: r0v110, types: [long] */
    /* JADX WARN: Type inference failed for: r0v43, types: [long] */
    /* JADX WARN: Type inference failed for: r0v46, types: [long] */
    /* JADX WARN: Type inference failed for: r0v55, types: [long] */
    /* JADX WARN: Type inference failed for: r0v59, types: [long] */
    /* JADX WARN: Type inference failed for: r0v62, types: [long] */
    /* JADX WARN: Type inference failed for: r0v70 */
    /* JADX WARN: Type inference failed for: r0v71 */
    /* JADX WARN: Type inference failed for: r0v89, types: [long] */
    /* JADX WARN: Type inference failed for: r0v92 */
    /* renamed from: a */
    public static dn2 m7809a(yy2 yy2Var) {
        char c;
        char c2;
        boolean z;
        char c3;
        char c4;
        char c5;
        char c6;
        char c7;
        long currentTimeMillis = System.currentTimeMillis();
        Map<String, String> map = yy2Var.f9287c;
        if (map == null) {
            return null;
        }
        String str = map.get("Date");
        char m7808b = str != null ? m7808b(str) : (char) 0;
        String str2 = map.get("Cache-Control");
        boolean z2 = false;
        int i = 0;
        if (str2 != null) {
            String[] split = str2.split(",", 0);
            z2 = false;
            c2 = 0;
            ?? r0 = 0;
            while (true) {
                c = r0;
                if (i >= split.length) {
                    z = true;
                    break;
                }
                String trim = split[i].trim();
                if (trim.equals("no-cache") || trim.equals("no-store")) {
                    return null;
                }
                if (trim.startsWith("max-age=")) {
                    try {
                        c6 = Long.parseLong(trim.substring(8));
                        c7 = c;
                    } catch (Exception e) {
                        c6 = c2;
                        c7 = c;
                    }
                } else if (trim.startsWith("stale-while-revalidate=")) {
                    c7 = Long.parseLong(trim.substring(23));
                    c6 = c2;
                } else {
                    if (!trim.equals("must-revalidate")) {
                        c6 = c2;
                        c7 = c;
                        if (!trim.equals("proxy-revalidate")) {
                        }
                    }
                    z2 = true;
                    c7 = c;
                    c6 = c2;
                }
                i++;
                c2 = c6;
                r0 = c7;
            }
        } else {
            z = false;
            c2 = 0;
            c = 0;
        }
        String str3 = map.get("Expires");
        char m7808b2 = str3 != null ? m7808b(str3) : (char) 0;
        String str4 = map.get("Last-Modified");
        char m7808b3 = str4 != null ? m7808b(str4) : (char) 0;
        String str5 = map.get("ETag");
        if (z) {
            ?? r02 = currentTimeMillis + (c2 * 1000);
            if (z2) {
                c5 = r02;
            } else {
                Long.signum(c);
                c5 = (c * 1000) + r02;
            }
            char c8 = c5;
            c3 = r02;
            c4 = c8;
        } else {
            c4 = 0;
            if (m7808b <= 0 || m7808b2 < m7808b) {
                c3 = 0;
            } else {
                c3 = currentTimeMillis + (m7808b2 - m7808b);
                c4 = c3;
            }
        }
        dn2 dn2Var = new dn2();
        dn2Var.f6342a = yy2Var.f9286b;
        dn2Var.f6343b = str5;
        dn2Var.f6347f = c3;
        dn2Var.f6346e = c4;
        dn2Var.f6344c = m7808b;
        dn2Var.f6345d = m7808b3;
        dn2Var.f6348g = map;
        dn2Var.f6349h = yy2Var.f9288d;
        return dn2Var;
    }

    /* renamed from: b */
    public static long m7808b(String str) {
        try {
            return m7806d("EEE, dd MMM yyyy HH:mm:ss zzz").parse(str).getTime();
        } catch (ParseException e) {
            if ("0".equals(str) || "-1".equals(str)) {
                C1718ec.m7660a("Unable to parse dateStr: %s, falling back to 0", str);
                return 0L;
            }
            C1718ec.m7657d(e, "Unable to parse dateStr: %s, falling back to 0", str);
            return 0L;
        }
    }

    /* renamed from: c */
    static String m7807c(long j) {
        return m7806d("EEE, dd MMM yyyy HH:mm:ss 'GMT'").format(new Date(j));
    }

    /* renamed from: d */
    private static SimpleDateFormat m7806d(String str) {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(str, Locale.US);
        simpleDateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
        return simpleDateFormat;
    }
}

...
public class FinderLazyLoader<T> {
    ...
    public T getFirstFromDb() {
        Table table = this.finderColumn.getTable();
        return table != null ? table.f11339db.findFirst(Selector.from(this.finderColumn.getTargetEntityType()).where(this.finderColumn.getTargetColumnName(), "=", this.finderValue)) : null;
    }
}

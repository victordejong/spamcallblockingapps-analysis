...
/* loaded from: classes-dex2jar.jar:com/google/android/exoplayer2/audio/AudioTrackPositionTracker.class */
public final class AudioTrackPositionTracker {
    ...
    /* loaded from: classes-dex2jar.jar:com/google/android/exoplayer2/audio/AudioTrackPositionTracker$Listener.class */
    ...
    private long framesToDurationUs(long j) {
        return (j * C0515C.MICROS_PER_SECOND) / this.outputSampleRate;
    }

    /* JADX WARN: Type inference failed for: r0v35, types: [long] */
    /* JADX WARN: Type inference failed for: r0v8, types: [long] */
    private long getPlaybackHeadPosition() {
        if (this.stopTimestampUs != C0515C.TIME_UNSET) {
            return Math.min(this.endPlaybackHeadPosition, this.stopPlaybackHeadPosition + ((((SystemClock.elapsedRealtime() * 1000) - this.stopTimestampUs) * this.outputSampleRate) / C0515C.MICROS_PER_SECOND));
        }
        ...
        if (this.needsPassthroughWorkarounds) {
            ...
            c = playbackHeadPosition + this.passthroughWorkaroundPauseOffset;
        }
        if (Util.SDK_INT <= 28) {
            if (c == 0 && this.lastRawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C0515C.TIME_UNSET) {
                    this.forceResetWorkaroundTimeMs = SystemClock.elapsedRealtime();
                }
                return this.lastRawPlaybackHeadPosition;
            }
            this.forceResetWorkaroundTimeMs = C0515C.TIME_UNSET;
        }
        if (this.lastRawPlaybackHeadPosition > c) {
            this.rawPlaybackHeadWrapCount++;
        }
        this.lastRawPlaybackHeadPosition = c;
        return c + (this.rawPlaybackHeadWrapCount << 32);
    }
    ...
    public long getCurrentPositionUs(boolean z) {
        ...
        char playbackHeadPositionUs = this.playheadOffsetCount == 0 ? getPlaybackHeadPositionUs() : nanoTime + this.smoothedPlayheadOffsetUs;
        char c = playbackHeadPositionUs;
        if (!z) {
            c = playbackHeadPositionUs - this.latencyUs;
        }
        return c;
    }
    ...
    public boolean isStalled(long j) {
        return this.forceResetWorkaroundTimeMs != C0515C.TIME_UNSET && j > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean mayHandleBuffer(long j) {
        ...
        listener.onUnderrun(this.bufferSize, C0515C.usToMs(this.bufferSizeUs));
        return true;
    }
    ...
    public void setAudioTrack(AudioTrack audioTrack, int i, int i2, int i3) {
        ...
        this.bufferSizeUs = isEncodingPcm ? framesToDurationUs(i3 / i2) : (char) 1;
        this.lastRawPlaybackHeadPosition = 0L;
        this.rawPlaybackHeadWrapCount = 0L;
        this.passthroughWorkaroundPauseOffset = 0L;
        this.hasData = false;
        this.stopTimestampUs = C0515C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C0515C.TIME_UNSET;
        this.latencyUs = 0L;
    }

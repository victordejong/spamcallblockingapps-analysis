...
/* loaded from: classes3-dex2jar.jar:org/bouncycastle/asn1/crmf/PKIArchiveOptions.class */
public class PKIArchiveOptions extends ASN1Encodable implements ASN1Choice {
    ...
    private PKIArchiveOptions(ASN1TaggedObject aSN1TaggedObject) {
        ...
        if (tagNo == 0) {
            dERBoolean = EncryptedKey.getInstance(aSN1TaggedObject.getObject());
        } else if (tagNo == 1) {
            dERBoolean = ASN1OctetString.getInstance(aSN1TaggedObject, false);
        } else if (tagNo != 2) {
            throw new IllegalArgumentException("unknown tag number: " + aSN1TaggedObject.getTagNo());
        } else {
            dERBoolean = DERBoolean.getInstance(aSN1TaggedObject, false);
        }
        this.value = dERBoolean;
    }

...
/* loaded from: classes3-dex2jar.jar:org/bouncycastle/x509/examples/AttrCertExample.class */
public class AttrCertExample {
    ...
    public static X509Certificate createClientCert(PublicKey publicKey, PrivateKey privateKey, PublicKey publicKey2) {
        ...
        DERObjectIdentifier dERObjectIdentifier3 = X509Name.f7272L;
        hashtable.put(dERObjectIdentifier3, "Melbourne");
        DERObjectIdentifier dERObjectIdentifier4 = X509Name.f7269CN;
        hashtable.put(dERObjectIdentifier4, "Eric H. Echidna");
        DERObjectIdentifier dERObjectIdentifier5 = X509Name.EmailAddress;
        hashtable.put(dERObjectIdentifier5, "feedback-crypto@bouncycastle.org");
        ...
        vector.addElement(dERObjectIdentifier5);
        v3CertGen.reset();
        ...
        v3CertGen.addExtension((DERObjectIdentifier) MiscObjectIdentifiers.netscapeCertType, false, (DEREncodable) new NetscapeCertType(48));
    ...
    public static void main(String[] strArr) {
        ...
        X509V2AttributeCertificate x509V2AttributeCertificate = (X509V2AttributeCertificate) x509V2AttributeCertificateGenerator.generate(generatePrivate, "BC");
        ...
        if (holder.match((Certificate) createClientCert)) {
            if (holder.getEntityNames() != null) {
                ...
                printStream.println(holder.getEntityNames().length + " entity names found");
            }
            if (holder.getIssuer() != null) {
                PrintStream printStream2 = System.out;
                printStream2.println(holder.getIssuer().length + " issuer names found, serial number " + holder.getSerialNumber());
            }
        ...
        if (issuer.match((Certificate) createAcIssuerCert)) {
            if (issuer.getPrincipals() != null) {
                ...
                printStream3.println(issuer.getPrincipals().length + " entity names found");
            }
            System.out.println("Matches original ca x509 cert");
        }
        PrintStream printStream4 = System.out;
        printStream4.println("valid not before: " + x509V2AttributeCertificate.getNotBefore());
        PrintStream printStream5 = System.out;
        printStream5.println("valid not before: " + x509V2AttributeCertificate.getNotAfter());
        try {
        ...
        try {
            x509V2AttributeCertificate.verify(generatePublic, "BC");
        } catch (Exception e2) {
            System.out.println(e2);
        }
        X509Attribute[] attributes = x509V2AttributeCertificate.getAttributes();
        PrintStream printStream6 = System.out;
        printStream6.println("cert has " + attributes.length + " attributes:");
        for (int i = 0; i < attributes.length; i++) {
            X509Attribute x509Attribute = attributes[i];
            PrintStream printStream7 = System.out;
            printStream7.println("OID: " + x509Attribute.getOID());

...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f21mX = new float[min];
        this.f22mY = new float[min];
    ...
    public float getInterpolation(float f) {
        ...
        float f2 = this.f21mX[length] - this.f21mX[i];
        if (f2 == 0.0f) {
            return this.f22mY[i];
        }
        float f3 = (f - this.f21mX[i]) / f2;
        float f4 = this.f22mY[i];
        return f4 + (f3 * (this.f22mY[length] - f4));
    }
}

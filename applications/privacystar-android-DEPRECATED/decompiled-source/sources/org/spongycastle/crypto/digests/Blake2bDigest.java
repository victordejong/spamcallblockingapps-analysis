...
/* loaded from: classes-dex2jar.jar:org/spongycastle/crypto/digests/Blake2bDigest.class */
public class Blake2bDigest implements ExtendedDigest {
    ...
    /* renamed from: t0 */
    private long f1047t0;

    /* renamed from: t1 */
    private long f1048t1;
    ...
    public Blake2bDigest(Blake2bDigest blake2bDigest) {
        ...
        this.f1047t0 = blake2bDigest.f1047t0;
        this.f1048t1 = blake2bDigest.f1048t1;
        this.f1046f0 = blake2bDigest.f1046f0;
    }
    ...
    /* renamed from: G */
    private void m194G(long j, long j2, int i, int i2, int i3, int i4) {
    ...
    private void compress(byte[] bArr, int i) {
        ...
        for (int i3 = 0; i3 < rOUNDS; i3++) {
            m194G(jArr[blake2b_sigma[i3][0]], jArr[blake2b_sigma[i3][1]], 0, 4, 8, 12);
            m194G(jArr[blake2b_sigma[i3][2]], jArr[blake2b_sigma[i3][3]], 1, 5, 9, 13);
            m194G(jArr[blake2b_sigma[i3][4]], jArr[blake2b_sigma[i3][5]], 2, 6, 10, 14);
            m194G(jArr[blake2b_sigma[i3][6]], jArr[blake2b_sigma[i3][7]], 3, 7, 11, 15);
            m194G(jArr[blake2b_sigma[i3][8]], jArr[blake2b_sigma[i3][9]], 0, 5, 10, 15);
            m194G(jArr[blake2b_sigma[i3][10]], jArr[blake2b_sigma[i3][11]], 1, 6, 11, 12);
            m194G(jArr[blake2b_sigma[i3][12]], jArr[blake2b_sigma[i3][13]], 2, 7, 8, 13);
            m194G(jArr[blake2b_sigma[i3][14]], jArr[blake2b_sigma[i3][15]], 3, 4, 9, 14);
        }
    ...
    private void init() {
        if (this.chainValue == null) {
            ...
            if (this.personalization != null) {
                ...
                jArr3[6] = bytes2long(this.personalization, 0) ^ jArr3[6];
    ...
    private void initializeInternalState() {
        ...
        this.internalState[12] = this.f1047t0 ^ blake2b_IV[4];
        this.internalState[13] = this.f1048t1 ^ blake2b_IV[5];
        this.internalState[14] = this.f1046f0 ^ blake2b_IV[6];
    ...
    public int doFinal(byte[] bArr, int i) {
        int i2;
        this.f1046f0 = -1L;
        this.f1047t0 += this.bufferPos;
        if (this.f1047t0 < 0 && this.bufferPos > (-this.f1047t0)) {
            this.f1048t1++;
        }
        compress(this.buffer, 0);
        Arrays.fill(this.buffer, (byte) 0);
        Arrays.fill(this.internalState, 0L);
        for (int i3 = 0; i3 < this.chainValue.length && (i2 = i3 * 8) < this.digestLength; i3++) {
            byte[] long2bytes = long2bytes(this.chainValue[i3]);
            if (i2 < this.digestLength - 8) {
                System.arraycopy(long2bytes, 0, bArr, i2 + i, 8);
            } else {
                System.arraycopy(long2bytes, 0, bArr, i + i2, this.digestLength - i2);
            }
        }

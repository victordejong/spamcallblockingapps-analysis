...
class ApptentiveNotificationObserverList {
    ...
    public static class ObserverWeakReference extends WeakReference<ApptentiveNotificationObserver> implements ApptentiveNotificationObserver {
        ...
        public void onReceiveNotification(ApptentiveNotification apptentiveNotification) {
            ApptentiveNotificationObserver apptentiveNotificationObserver = (ApptentiveNotificationObserver) get();
    ...
    private int indexOf(ApptentiveNotificationObserver apptentiveNotificationObserver) {
        for (int i = 0; i < this.observers.size(); i++) {
            ...
            ObserverWeakReference observerWeakReference = (ObserverWeakReference) ObjectUtils.m395as(apptentiveNotificationObserver2, ObserverWeakReference.class);
    ...
    public void notifyObservers(ApptentiveNotification apptentiveNotification) {
        ...
        for (int i = 0; i < this.observers.size(); i++) {
            ...
            ObserverWeakReference observerWeakReference = (ObserverWeakReference) ObjectUtils.m395as(apptentiveNotificationObserver, ObserverWeakReference.class);
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            try {
                ...
                ApptentiveLog.m408e(e, "Exception while posting notification: %s", apptentiveNotification);
            }
        }
        if (z) {
            for (int size = this.observers.size() - 1; size >= 0; size--) {
                ObserverWeakReference observerWeakReference2 = (ObserverWeakReference) ObjectUtils.m395as(this.observers.get(size), ObserverWeakReference.class);

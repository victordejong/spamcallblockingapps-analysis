...
public class ApptentiveToastNotificationManager {
        ...
        synchronized (this) {
            if (!this.pendingNotificationQueue.isEmpty()) {
                ...
                if (poll.getCustomView() == null && poll.isActivateStatusBar()) {
                    ...
                    this.notificationManager.notify(poll.getCode(), poll.getNotificationBuilder().setIcon(poll.getIcon()).build());
                }
    ...
    private void showFloatingNotification(ApptentiveToastNotification apptentiveToastNotification) {
        ...
        layoutParams.x = this.tContainer.originalLeft;
        layoutParams.y = 0;
        layoutParams.alpha = 1.0f;
        try {
            this.wManager.addView(this.tContainer, layoutParams);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void cleanUp() {
        ...
        if (!(this.tContainer == null || this.tContainer.getParent() == null)) {
            this.wManager.removeView(this.tContainer);
        }
        this.wManager = null;
        this.notificationManager = null;
    ...
    public void remove(int i) {
        if (this.codeToNotificationMap.containsKey(Integer.valueOf(i))) {
            this.pendingNotificationQueue.remove(this.codeToNotificationMap.get(Integer.valueOf(i)));
        }
        if (this.tContainer != null && this.tContainer.getApptentiveFloatingNotification().getCode() == i) {
            startDismissalAnimation();
        }
    }

    public void remove(ApptentiveToastNotification apptentiveToastNotification) {
        synchronized (this) {
            remove(apptentiveToastNotification.getCode());
        }
    }
    ...
    public void startDismissalAnimationAt(ApptentiveToastNotification apptentiveToastNotification) {
        if (this.tContainer != null && this.tContainer.getParent() != null && this.tContainer.getApptentiveFloatingNotification().getCode() == apptentiveToastNotification.getCode()) {
            startDismissalAnimation();
        }
    }

...
public class SessionAnalyticsManager implements BackgroundManager.Listener {
    ...
    public void onBackground() {
        Fabric.getLogger().mo288d(Answers.TAG, "Flush events when app is backgrounded");
    ...
    public void onCrash(String str, String str2) {
        ...
        Fabric.getLogger().mo288d(Answers.TAG, "Logged crash");
        this.eventsHandler.processEventSync(SessionEvent.crashEventBuilder(str, str2));
    }

    public void onCustom(CustomEvent customEvent) {
        Logger logger = Fabric.getLogger();
        logger.mo288d(Answers.TAG, "Logged custom event: " + customEvent);
    ...
    public void onInstall(long j) {
        Fabric.getLogger().mo288d(Answers.TAG, "Logged install");
        this.eventsHandler.processEventAsyncAndFlush(SessionEvent.installEventBuilder(j));
    }

    public void onLifecycle(Activity activity, SessionEvent.Type type) {
        Logger logger = Fabric.getLogger();
        logger.mo288d(Answers.TAG, "Logged lifecycle event: " + type.name());
        this.eventsHandler.processEventAsync(SessionEvent.lifecycleEventBuilder(type, activity));
    }

    public void onPredefined(PredefinedEvent predefinedEvent) {
        Logger logger = Fabric.getLogger();
        logger.mo288d(Answers.TAG, "Logged predefined event: " + predefinedEvent);

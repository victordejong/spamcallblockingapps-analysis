...
/* loaded from: classes4-dex2jar.jar:com/google/firebase/crashlytics/internal/persistence/CrashlyticsReportPersistence.class */
public class CrashlyticsReportPersistence {
    ...
    private static void synthesizeNativeReportFile(File file, File file2, CrashlyticsReport.FilesPayload filesPayload, String str) {
        try {
            ...
            Logger.getLogger().m8479w("Could not synthesize final native report file for ".concat(String.valueOf(file)), e);
        }
    }
    ...
    private void synthesizeReport(File file, long j) {
        ...
        if (filesInDirectory.isEmpty()) {
            Logger.getLogger().m8482v("Session " + file.getName() + " has no events.");
            return;
        }
        ...
        loop0: while (true) {
            ...
            for (File file2 : filesInDirectory) {
                try {
                    ...
                    Logger.getLogger().m8479w("Could not add event to report for ".concat(String.valueOf(file2)), e);
                }
        ...
        if (arrayList.isEmpty()) {
            Logger.getLogger().m8480w("Could not parse event files for session " + file.getName());
            return;
        }
        ...
        if (file3.isFile()) {
            try {
                ...
                Logger.getLogger().m8479w("Could not read user ID file in " + file.getName(), e2);
                str = null;
            }
        }
    ...
    private static void synthesizeReportFile(File file, File file2, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        try {
            ...
            Logger.getLogger().m8479w("Could not synthesize final report file for ".concat(String.valueOf(file)), e);
        }
    }
    ...
    public void finalizeReports(String str, long j) {
        for (File file : capAndGetOpenSessions(str)) {
            ...
            logger.m8482v("Finalizing report for session " + file.getName());
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : getAllFinalizedReportFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.m8479w("Could not load report file " + file + "; deleting", e);
                file.delete();
            }
        }
        return arrayList;
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger.getLogger().m8479w("Could not persist event for session ".concat(String.valueOf(str)), e);
        }
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().m8488d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            Logger.getLogger().m8487d("Could not persist report for session ".concat(String.valueOf(identifier)), e);
        }
    }
    ...
    public void persistUserIdForSession(String str, String str2) {
        try {
            ...
            Logger.getLogger().m8479w("Could not persist user ID for session ".concat(String.valueOf(str2)), e);
        }
    }
}

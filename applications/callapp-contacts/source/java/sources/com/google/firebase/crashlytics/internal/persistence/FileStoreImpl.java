...
import java.io.File;
/* loaded from: classes4-dex2jar.jar:com/google/firebase/crashlytics/internal/persistence/FileStoreImpl.class */
public class FileStoreImpl implements FileStore {
    ...
    boolean isExternalStorageAvailable() {
        if (!"mounted".equals(Environment.getExternalStorageState())) {
            Logger.getLogger().m8480w("External Storage is not mounted and/or writable\nHave you declared android.permission.WRITE_EXTERNAL_STORAGE in the manifest?");
            return false;
        }
        return true;
    }
    ...
    File prepare(File file) {
        if (file == null) {
            Logger.getLogger().m8480w("Null File");
            return null;
        } else if (file.exists() || file.mkdirs()) {
            return file;
        } else {
            Logger.getLogger().m8480w("Couldn't create file");
            return null;
        }
    }
}

...
public class CrashlyticsCore {
    ...
    public CrashlyticsCore(C15601b c15601b, IdManager idManager, CrashlyticsNativeComponent crashlyticsNativeComponent, DataCollectionArbiter dataCollectionArbiter, BreadcrumbSource breadcrumbSource, AnalyticsEventLogger analyticsEventLogger, ExecutorService executorService) {
        this.app = c15601b;
        this.dataCollectionArbiter = dataCollectionArbiter;
        this.context = c15601b.m8583a();
    ...
    public AbstractC14185h<Void> doBackgroundInitialization(SettingsDataProvider settingsDataProvider) {
        ...
        try {
            ...
            if (!settingsDataProvider.getSettings().getFeaturesData().collectReports) {
                Logger.getLogger().m8488d("Collection of crash reports disabled in Crashlytics settings.");
                return C14188k.m11465a((Exception) new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions()) {
                Logger.getLogger().m8480w("Previous sessions could not be finalized.");
            }
            return this.controller.submitAllReports(settingsDataProvider.getAppSettings());
        } catch (Exception e) {
            Logger.getLogger().m8485e("Crashlytics encountered a problem during asynchronous initialization.", e);
            return C14188k.m11465a(e);
        } finally {
    ...
    private void finishInitSynchronously(final SettingsDataProvider settingsDataProvider) {
        ...
        Logger.getLogger().m8488d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            submit.get(4L, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().m8485e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().m8485e("Crashlytics encountered a problem during initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().m8485e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().m8482v("Configured not to require a build ID.");
            return true;
    ...
    public AbstractC14185h<Void> doBackgroundInitializationAsync(final SettingsDataProvider settingsDataProvider) {
        return Utils.callTask(this.crashHandlerExecutor, new Callable<AbstractC14185h<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.1
            @Override // java.util.concurrent.Callable
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.3
            ...
            public Boolean call() throws Exception {
                try {
                    ...
                    if (!remove) {
                        Logger.getLogger().m8480w("Initialization marker file was not properly removed.");
                    }
                    return Boolean.valueOf(remove);
                } catch (Exception e) {
                    Logger.getLogger().m8485e("Problem encountered deleting Crashlytics initialization marker.", e);
    ...
    void markInitializationStarted() {
        ...
        Logger.getLogger().m8482v("Initialization marker file was created.");
    }
        ...
        if (isBuildIdValid(appData.buildId, CommonUtils.getBooleanResourceValue(this.context, CRASHLYTICS_REQUIRE_BUILD_ID, true))) {
            try {
                ...
                if (!didPreviousInitializationFail || !CommonUtils.canTryConnection(this.context)) {
                    Logger.getLogger().m8488d("Successfully configured exception handler.");
                    return true;
                }
                Logger.getLogger().m8488d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsDataProvider);
                return false;
            } catch (Exception e) {
                Logger.getLogger().m8485e("Crashlytics was not started due to an exception during initialization", e);

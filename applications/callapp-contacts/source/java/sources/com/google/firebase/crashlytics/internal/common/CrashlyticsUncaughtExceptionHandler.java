...
public class CrashlyticsUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable th) {
        ...
        try {
            try {
                if (thread == null) {
                    Logger.getLogger().m8486e("Could not handle uncaught exception; null thread");
                } else if (th == null) {
                    Logger.getLogger().m8486e("Could not handle uncaught exception; null throwable");
                } else {
                    this.crashListener.onUncaughtException(this.settingsDataProvider, thread, th);
                }
            } catch (Exception e) {
                Logger.getLogger().m8485e("An error occurred in the uncaught exception handler", e);
            }
            Logger.getLogger().m8488d("Completed exception processing. Invoking default exception handler.");

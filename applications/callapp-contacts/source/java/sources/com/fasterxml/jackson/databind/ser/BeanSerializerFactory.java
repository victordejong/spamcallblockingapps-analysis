...
public class BeanSerializerFactory extends BasicSerializerFactory implements Serializable {
    ...
    */
    ...
    public JsonSerializer<Object> createSerializer(SerializerProvider serializerProvider, JavaType javaType) throws JsonMappingException {
        ...
        Converter<Object, Object> findSerializationConverter = beanDescription.findSerializationConverter();
        if (findSerializationConverter == null) {
            return _createSerializer2(serializerProvider, refineSerializationType, beanDescription, z);
        }
        ...
        if (!outputType.hasRawClass(refineSerializationType.getRawClass())) {
            beanDescription2 = config.introspect(outputType);
            jsonSerializer = findSerializerFromAnnotation(serializerProvider, beanDescription2.getClassInfo());
        }
        JsonSerializer<?> jsonSerializer2 = jsonSerializer;
        if (jsonSerializer == null) {
            jsonSerializer2 = jsonSerializer;
            if (!outputType.isJavaLangObject()) {
                jsonSerializer2 = _createSerializer2(serializerProvider, outputType, beanDescription2, true);
            }
        }
    ...
    protected List<BeanPropertyWriter> removeOverlappingTypeIds(SerializerProvider serializerProvider, BeanDescription beanDescription, BeanSerializerBuilder beanSerializerBuilder, List<BeanPropertyWriter> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (typeSerializer != null && typeSerializer.getTypeInclusion() == JsonTypeInfo.EnumC10517As.EXTERNAL_PROPERTY) {

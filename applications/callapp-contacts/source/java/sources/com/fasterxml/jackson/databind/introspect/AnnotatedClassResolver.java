...
public class AnnotatedClassResolver {
    ...
    AnnotatedClassResolver(MapperConfig<?> mapperConfig, JavaType javaType, ClassIntrospector.MixInResolver mixInResolver) {
        ...
        this._primaryMixin = mixInResolver == null ? null : mixInResolver.findMixInClassFor(rawClass);
    ...
    private AnnotationCollector _addAnnotationsIfNotPresent(AnnotationCollector annotationCollector, Annotation[] annotationArr) {
        ...
        if (annotationArr != null) {
            ...
            int i = 0;
            while (true) {
                AnnotationCollector annotationCollector3 = annotationCollector;
                annotationCollector2 = annotationCollector3;
                if (i >= length) {
                    break;
                }
                Annotation annotation = annotationArr[i];
                annotationCollector = annotationCollector3;
                if (!annotationCollector3.isPresent(annotation)) {
                    AnnotationCollector addOrOverride = annotationCollector3.addOrOverride(annotation);
    ...
    private Annotations resolveClassAnnotations(List<JavaType> list) {
        ...
        for (JavaType javaType : list) {
            AnnotationCollector annotationCollector4 = annotationCollector3;
            if (z) {
                Class<?> rawClass = javaType.getRawClass();
                annotationCollector4 = _addClassMixIns(annotationCollector3, rawClass, this._mixInResolver.findMixInClassFor(rawClass));
            }
            annotationCollector3 = annotationCollector4;
            if (this._collectAnnotations) {
                annotationCollector3 = _addAnnotationsIfNotPresent(annotationCollector4, ClassUtil.findClassAnnotations(javaType.getRawClass()));
            }
        }
        AnnotationCollector annotationCollector5 = annotationCollector3;
        if (z) {
            annotationCollector5 = _addClassMixIns(annotationCollector3, Object.class, this._mixInResolver.findMixInClassFor(Object.class));
        }
        return annotationCollector5.asAnnotations();
    }

...
public class TypeFactory implements Serializable {
    ...
    private JavaType _mapType(Class<?> cls, TypeBindings typeBindings, JavaType javaType, JavaType[] javaTypeArr) {
        ...
        if (cls == Properties.class) {
            ...
            if (size != 0) {
                ...
                simpleType = typeParameters.get(0);
                javaType2 = typeParameters.get(1);
                return MapType.construct(cls, typeBindings, javaType, javaTypeArr, simpleType, javaType2);
            }
            simpleType = _unknownType();
        }
    ...
    public JavaType _fromClass(ClassStack classStack, Class<?> cls, TypeBindings typeBindings) {
        ...
        MapType mapType = this._typeCache.get(asKey);
        if (mapType != null) {
            return mapType;
        }
        if (classStack == null) {
        ...
        if (cls.isArray()) {
            arrayType = ArrayType.construct(_fromAny(classStack2, cls.getComponentType(), typeBindings), typeBindings);
        } else {
            if (cls.isInterface()) {
                javaType = null;
            ...
            if (cls == Properties.class) {
                ...
                mapType = MapType.construct(cls, typeBindings, javaType, javaTypeArr, simpleType, simpleType);
            } else if (javaType != null) {
                mapType = javaType.refine(cls, typeBindings, javaType, javaTypeArr);
            }
            arrayType = mapType;
            if (mapType == null) {
                JavaType _fromWellKnownClass = _fromWellKnownClass(classStack2, cls, typeBindings, javaType, javaTypeArr);
                arrayType = _fromWellKnownClass;
                if (_fromWellKnownClass == null) {
                    JavaType _fromWellKnownInterface = _fromWellKnownInterface(classStack2, cls, typeBindings, javaType, javaTypeArr);
                    arrayType = _fromWellKnownInterface;
                    if (_fromWellKnownInterface == null) {
                        arrayType = _newSimpleType(cls, typeBindings, javaType, javaTypeArr);
                    }
                }
            }
        }
        classStack2.resolveSelfReferences(arrayType);
        if (!arrayType.hasHandlers()) {
            this._typeCache.putIfAbsent(asKey, arrayType);
        }
        return arrayType;
    }
    ...
    public MapType constructMapType(Class<? extends Map> cls, Class<?> cls2, Class<?> cls3) {
        ...
        if (cls == Properties.class) {
            ...
            simpleType = _fromClass(null, cls2, typeBindings);
            javaType = _fromClass(null, cls3, typeBindings);
        }
        return constructMapType(cls, simpleType, javaType);
    }

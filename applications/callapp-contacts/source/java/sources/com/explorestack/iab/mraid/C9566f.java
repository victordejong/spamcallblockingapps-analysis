package com.explorestack.iab.mraid;

import android.app.Activity;
import android.content.Context;
import android.text.TextUtils;
import android.util.Base64;
import android.util.Pair;
import android.view.View;
import android.view.ViewGroup;
import com.explorestack.iab.IabSettings;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Iterator;
import java.util.List;
import java.util.concurrent.TimeUnit;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
import net.pubnative.lite.sdk.vpaid.enums.EventConstants;
/* renamed from: com.explorestack.iab.mraid.f */
/* loaded from: classes2-dex2jar.jar:com/explorestack/iab/mraid/f.class */
public class C9566f {

    /* renamed from: b */
    static final /* synthetic */ boolean f32516b = true;

    /* renamed from: d */
    private static String f32518d;

    /* renamed from: a */
    public static final long f32515a = TimeUnit.SECONDS.toMillis(30);

    /* renamed from: c */
    private static final List<String> f32517c = Arrays.asList(EventConstants.CLOSE, "expand", "open", "playVideo", "resize", "setOrientationProperties", "setResizeProperties", "useCustomClose", "noFill", "loaded", "AdStarted", "AdStopped", "AdSkipped", "AdSkippableStateChange", "AdVideoStart", "AdVideoFirstQuartile", "AdVideoMidpoint", "AdVideoThirdQuartile", "AdVideoComplete", "AdUserClose", "AdPaused", "AdPlaying", "AdClickThru", "AdLog", "AdError");

    /* renamed from: a */
    private static View m24171a(Context context) {
        if (!(context instanceof Activity)) {
            return null;
        }
        return ((Activity) context).getWindow().getDecorView().findViewById(16908290);
    }

    /* renamed from: a */
    public static View m24170a(Context context, View view) {
        View m24165b = m24165b(context, view);
        return m24165b instanceof ViewGroup ? (ViewGroup) m24165b : view;
    }

    /* renamed from: a */
    private static View m24169a(View view) {
        View rootView;
        if (view == null || (rootView = view.getRootView()) == null) {
            return null;
        }
        View findViewById = rootView.findViewById(16908290);
        return findViewById != null ? findViewById : rootView;
    }

    /* renamed from: a */
    public static String m24172a() {
        if (f32518d == null) {
            f32518d = new String(Base64.decode("", 0)).replace("%%VERSION%%", IabSettings.mediatorVersion);
        }
        return f32518d;
    }

    /* renamed from: a */
    private static List<Pair<Integer, Integer>> m24167a(String str, char... cArr) {
        ArrayList arrayList = new ArrayList();
        if (TextUtils.isEmpty(str)) {
            return arrayList;
        }
        if (!f32516b && str == null) {
            throw new AssertionError();
        }
        try {
            int[] iArr = new int[2];
            for (int i = 0; i < str.length(); i++) {
                char charAt = str.charAt(i);
                int i2 = 0;
                while (true) {
                    if (i2 >= 2) {
                        break;
                    } else if (charAt == cArr[i2]) {
                        int i3 = iArr[i2];
                        if (i3 > 0) {
                            arrayList.add(new Pair(Integer.valueOf(i3), Integer.valueOf(i)));
                            iArr[i2] = 0;
                        } else {
                            iArr[i2] = i;
                        }
                    } else {
                        i2++;
                    }
                }
            }
        } catch (Throwable th) {
        }
        return arrayList;
    }

    /* JADX WARN: Code restructure failed: missing block: B:49:0x01c8, code lost:
        if (r0.containsKey("allowOffscreen") != false) goto L56;
     */
    /* JADX WARN: Code restructure failed: missing block: B:53:0x01e3, code lost:
        if (r0.containsKey("forceOrientation") != false) goto L56;
     */
    /* renamed from: a */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
    public static java.util.Map<java.lang.String, java.lang.String> m24168a(java.lang.String r6) {
        /*
            Method dump skipped, instructions count: 576
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: com.explorestack.iab.mraid.C9566f.m24168a(java.lang.String):java.util.Map");
    }

    /* renamed from: a */
    private static void m24166a(StringBuffer stringBuffer) {
        boolean z;
        if (TextUtils.isEmpty(stringBuffer)) {
            return;
        }
        Matcher matcher = Pattern.compile("<head[^>]*>", 2).matcher(stringBuffer);
        List<Pair<Integer, Integer>> m24167a = m24167a(stringBuffer.toString(), '\"', '\'');
        while (matcher.find()) {
            int end = matcher.end();
            Iterator<Pair<Integer, Integer>> it2 = m24167a.iterator();
            while (true) {
                if (!it2.hasNext()) {
                    z = true;
                    break;
                }
                Pair<Integer, Integer> next = it2.next();
                if (end >= ((Integer) next.first).intValue() && end <= ((Integer) next.second).intValue()) {
                    z = false;
                    break;
                }
            }
            if (z) {
                stringBuffer.insert(end, "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no\" /><style>body { margin:0; padding:0;}*:not(input) { -webkit-touch-callout:none; -webkit-user-select:none; -webkit-text-size-adjust:none; }</style>");
                return;
            }
        }
    }

    /* renamed from: b */
    public static View m24165b(Context context, View view) {
        View m24171a = m24171a(context);
        return m24171a != null ? m24171a : m24169a(view);
    }

    /* renamed from: b */
    public static String m24164b(String str) {
        StringBuffer stringBuffer = new StringBuffer(str);
        Matcher matcher = Pattern.compile("<script\\s+[^>]*\\bsrc\\s*=\\s*([\\\"\\'])mraid\\.js\\1[^>]*>\\s*</script>\\n*", 2).matcher(stringBuffer);
        if (matcher.find()) {
            stringBuffer.delete(matcher.start(), matcher.end());
        }
        boolean contains = str.contains("<html");
        boolean contains2 = str.contains("<head");
        boolean contains3 = str.contains("<body");
        if ((contains || (!contains2 && !contains3)) && (!contains || contains3)) {
            if (!contains) {
                stringBuffer.insert(0, "<html><head></head><body><div align=\"center\">");
                stringBuffer.append("</div></body></html>");
            } else if (!contains2) {
                Matcher matcher2 = Pattern.compile("<html[^>]*>", 2).matcher(stringBuffer);
                for (int i = 0; matcher2.find(i); i = matcher2.end()) {
                    stringBuffer.insert(matcher2.end(), "<head></head>");
                }
            }
            m24166a(stringBuffer);
            return stringBuffer.toString();
        }
        return str;
    }
}

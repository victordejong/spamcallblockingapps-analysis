...
public class DeepLinkManager {
    ...
    public static boolean handleDeepLink(Context context, Intent intent, Class cls) {
        ...
        if (data != null) {
            ...
            if (StringUtils.m26045b((CharSequence) scheme) && DEEP_LINK_SCHEME_CALLAPP_CALLBACK.equalsIgnoreCase(scheme)) {
                String host = data.getHost();
                if (SCHEME_OPEN_CALLAPP_PLUS.equalsIgnoreCase(host)) {
                    Activities.m27656b(context, new Intent(context, CallAppPlusActivity.class));
                    return true;
                } else if (SCHEME_PRIVACY_POLICY.equalsIgnoreCase(host)) {
                    Activities.m27655b(context, Activities.m27697a(2131887432, HttpUtils.getCallAppDomain()));
                } else if (SCHEME_TERMS_OF_SERVICE.equalsIgnoreCase(host)) {
                    Activities.m27655b(context, Activities.m27697a(2131887848, HttpUtils.getCallAppDomain()));
                }
            }
        }
        ...
        if (StringUtils.m26059a((CharSequence) intentToActivityPath)) {
            CLog.m27606a(TAG);
            return false;
        }
        ...
        if (deepLinkClass == null) {
            CLog.m27606a(TAG);
            return false;
        } else if (!Activities.m27672a(intent)) {
            return true;
        } else {

...
public class EmailAction extends LocalAction {
    ...
    public final void mo31800b(final Context context, ContactData contactData, BaseAdapterItemData baseAdapterItemData) {
        AnalyticsManager.get().m28449a(Constants.ACTIONS, "Email action", Constants.CLICK);
        Collection<JSONEmail> visibleEmails = contactData.getVisibleEmails();
        if (visibleEmails.isEmpty()) {
            Activities.m27675a(context, (String[]) null, "");
            return;
        }
        int i = 0;
        if (visibleEmails.size() == 1) {
            Activities.m27675a(context, new String[]{visibleEmails.iterator().next().getEmail()}, "");
            return;
        }
        if (baseAdapterItemData != null) {
            for (JSONEmail jSONEmail : visibleEmails) {
                if (jSONEmail.getEmail().equals(baseAdapterItemData.displayName)) {
                    Activities.m27675a(context, new String[]{jSONEmail.getEmail()}, "");
                    return;
                }
            }
        }
        ...
        adapterText.setListener(new AdapterText.AdapterEvents() { // from class: com.callapp.contacts.action.local.EmailAction.1
            ...
            public void onRowClicked(int i2) {
                ...
                Activities.m27675a(context, new String[]{strArr[i2]}, "");
            }
        });

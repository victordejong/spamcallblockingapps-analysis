package com.dropbox.core.p265v2.sharing;

import com.dropbox.core.C8621n;
import com.dropbox.core.DbxApiException;
import java.util.Objects;
/* renamed from: com.dropbox.core.v2.sharing.ListFileMembersErrorException */
/* loaded from: classes2-dex2jar.jar:com/dropbox/core/v2/sharing/ListFileMembersErrorException.class */
public class ListFileMembersErrorException extends DbxApiException {

    /* renamed from: a */
    public final C9009ad f31373a;

    public ListFileMembersErrorException(String str, String str2, C8621n c8621n, C9009ad c9009ad) {
        super(str2, c8621n, m25359a(str, c8621n, c9009ad));
        Objects.requireNonNull(c9009ad, "errorValue");
        this.f31373a = c9009ad;
    }
}

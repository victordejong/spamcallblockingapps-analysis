package com.dropbox.core.p265v2.sharing;

import com.dropbox.core.C8621n;
import com.dropbox.core.DbxApiException;
import java.util.Objects;
/* renamed from: com.dropbox.core.v2.sharing.FileMemberActionErrorException */
/* loaded from: classes2-dex2jar.jar:com/dropbox/core/v2/sharing/FileMemberActionErrorException.class */
public class FileMemberActionErrorException extends DbxApiException {

    /* renamed from: a */
    public final C9186n f31368a;

    public FileMemberActionErrorException(String str, String str2, C8621n c8621n, C9186n c9186n) {
        super(str2, c8621n, m25359a(str, c8621n, c9186n));
        Objects.requireNonNull(c9186n, "errorValue");
        this.f31368a = c9186n;
    }
}

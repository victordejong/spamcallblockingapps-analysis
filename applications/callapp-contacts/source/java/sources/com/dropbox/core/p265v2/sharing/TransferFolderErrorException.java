package com.dropbox.core.p265v2.sharing;

import com.dropbox.core.C8621n;
import com.dropbox.core.DbxApiException;
import java.util.Objects;
/* renamed from: com.dropbox.core.v2.sharing.TransferFolderErrorException */
/* loaded from: classes2-dex2jar.jar:com/dropbox/core/v2/sharing/TransferFolderErrorException.class */
public class TransferFolderErrorException extends DbxApiException {

    /* renamed from: a */
    public final C9127bp f31390a;

    public TransferFolderErrorException(String str, String str2, C8621n c8621n, C9127bp c9127bp) {
        super(str2, c8621n, m25359a(str, c8621n, c9127bp));
        Objects.requireNonNull(c9127bp, "errorValue");
        this.f31390a = c9127bp;
    }
}

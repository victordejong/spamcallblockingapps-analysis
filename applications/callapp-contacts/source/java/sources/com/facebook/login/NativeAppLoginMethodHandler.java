...
public abstract class NativeAppLoginMethodHandler extends LoginMethodHandler {
    ...
    private LoginClient.Result handleResultCancel(LoginClient.Request request, Intent intent) {
        ...
        return C10210ac.m23255d().equals(obj) ? LoginClient.Result.createErrorResult(request, error, getErrorMessage(extras), obj) : LoginClient.Result.createCancelResult(request, error);
    }
    ...
    private LoginClient.Result handleResultOk(LoginClient.Request request, Intent intent) {
        ...
        if (error == null && obj == null && errorMessage == null) {
            try {
                return LoginClient.Result.createTokenResult(request, createAccessTokenFromWebBundle(request.getPermissions(), extras, EnumC10164c.FACEBOOK_APPLICATION_WEB, request.getApplicationId()));
            ...
            return null;
        } else if (C10210ac.m23257b().contains(error)) {
            return null;
        } else {
            return C10210ac.m23256c().contains(error) ? LoginClient.Result.createCancelResult(request, null) : LoginClient.Result.createErrorResult(request, error, errorMessage, obj);
        }
    }

...
public abstract class TimerWithPause {
    ...
    public void handleTimerMessage() {
        synchronized (this) {
            ...
            if (timeLeft <= 0) {
                ...
                onTick(timeLeft);
                char elapsedRealtime2 = this.mCountdownInterval - (SystemClock.elapsedRealtime() - elapsedRealtime);
    ...
    public long timeLeft() {
        char c;
        if (isPaused()) {
            c = this.mPauseTimeRemaining;
        } else {
            c = this.mStopTimeInFuture - SystemClock.elapsedRealtime();
            if (c < 0) {
                c = 0;
            }
        }

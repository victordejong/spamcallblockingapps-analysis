...
public class StandardEdition {
    ...
    private static final Logger f35361a = Logger.getInstance(StandardEdition.class);
    ...
    private static boolean a() {
        try {
            ...
            f35361a.e("Flurry Analytics library not found", e);
            return false;
        }
    }
    ...
    private static boolean a(Context context) {
        ...
        if (trim == null) {
            f35361a.i("No Flurry Analytics api-key provided.");
            return true;
        } else if (TextUtils.isEmpty(trim)) {
            f35361a.e("Unable to initialize Flurry Analytics. The flurry.api-key is empty.");
            return false;
        } else if (!a()) {
            f35361a.e("Unable to initialize Flurry Analytics. The flurry.api-key has been set but no Flurry Analytics library can be found.");
            return false;
        } else {
            try {
                if (FlurryAgent.isSessionActive()) {
                    f35361a.v("Flurry Analytics session already initialized.");
                    return true;
                }
                ...
                if (bool == null) {
                    f35361a.e("Unable to initialize Flurry Analytics. The flurry.isGdprScope key must be set.");
                    return false;
                }
                ...
                if (Logger.isLogLevelEnabled(3)) {
                    Logger logger2 = f35361a;
                    logger2.d("Flurry Analytics api-key is set to ".concat(String.valueOf(trim)));
                    logger2.d("Flurry Analytics isGdprScope is set to ".concat(String.valueOf(bool)));
                    logger2.d("Flurry Analytics consentStrings is set to ".concat(String.valueOf(map)));
                    logger2.d("Flurry Analytics dataSaleOptOut is set to ".concat(String.valueOf(z)));
                }
                builder.build(context, trim);
                FlurryAgent.addOrigin("vas", String.format("%s-%s", "standard-edition", BuildConfig.VERSION_NAME));
                return true;
            } catch (IllegalArgumentException e) {
                f35361a.e("Unable to initialize Flurry Analytics. Invalid flurry.api-key.", e);
                return false;
            } catch (Exception e2) {
                f35361a.e("Unable to initialize Flurry Analytics.", e2);
                return false;
            }
        }
    }
    ...
    public static boolean initialize(Application application, String str) {
        if (TextUtils.isEmpty(str)) {
            f35361a.e("siteId cannot be null or empty.");
            return false;
        }
        ...
        f35361a.e("Flurry Analytics initialization failed. Unable to initialize Verizon Ads SDK.");
        return false;
    }
}

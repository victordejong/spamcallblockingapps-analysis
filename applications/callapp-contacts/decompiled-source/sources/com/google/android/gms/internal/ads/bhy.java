package com.google.android.gms.internal.ads;
/* loaded from: classes3-dex2jar.jar:com/google/android/gms/internal/ads/bhy.class */
final /* synthetic */ class bhy {

    /* renamed from: a  reason: collision with root package name */
    static final /* synthetic */ int[] f24485a;

    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:10:0x002c -> B:16:0x001d). Please submit an issue!!! */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:9:0x0028 -> B:12:0x0013). Please submit an issue!!! */
    static {
        int[] iArr = new int[cpg.a().length];
        f24485a = iArr;
        try {
            iArr[cpg.f26210a - 1] = 1;
        } catch (NoSuchFieldError e) {
        }
        try {
            f24485a[cpg.f26211b - 1] = 2;
        } catch (NoSuchFieldError e2) {
        }
        try {
            f24485a[cpg.f26212c - 1] = 3;
        } catch (NoSuchFieldError e3) {
        }
    }
}

...
public class UpdateFieldBuilder<T> {
    ...
    public boolean execute(boolean z) {
        ...
        if (isValueChangedAndCanBeUse || z) {
            CLog.b(UpdateFieldBuilder.class, String.format("Updating %s.%s from data source: %s (old data source: %s)", this.contact.getId(), this.fieldName, this.dataSource, this.contact.getDataSource(this.field)), new Object[0]);

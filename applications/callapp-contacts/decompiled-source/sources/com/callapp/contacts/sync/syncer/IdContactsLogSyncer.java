...
public class IdContactsLogSyncer extends Syncer {
    ...
    public static void b() {
        ...
        boolean booleanValue = Prefs.i.get().booleanValue();
        ...
        inSync2.addSyncLoader(new CompoundAsyncLoader().a(compoundSyncLoader).a(new DeviceDataLoader()));
        ...
        for (ExtractedInfo extractedInfo : IMDataExtractionUtils.getAllImNotificationData()) {
            ContactData load = inSync.load(PhoneManager.get().a(extractedInfo.phoneAsRaw));
            if (load.getFastCacheData() == null || !StringUtils.b((CharSequence) load.getFastCacheData().getFullName()) || load.getFastCacheData().getPhotoUrls() == null || !StringUtils.b((CharSequence) load.getFastCacheData().getPhotoUrls().getPhotoUrl())) {
                ContactData load2 = inSync2.load(PhoneManager.get().a(extractedInfo.phoneAsRaw));
                if (StringUtils.b((CharSequence) load2.getFullName())) {
                    FastCacheDataManager.a(load2);
                }
                if (load2.hasAnyPhotoUrl()) {
                    BaseSocialSyncer.a(load2);
                }
            }
        }
    }

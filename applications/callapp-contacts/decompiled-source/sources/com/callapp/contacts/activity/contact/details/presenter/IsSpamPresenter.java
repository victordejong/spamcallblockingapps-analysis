...
public class IsSpamPresenter extends BaseUserCorrectedInfoPresenter implements CallStateListener, ContactDataChangeListener {
    ...
    private void expandIsSpamView(final Phone phone, ContactData contactData) {
        if (!contactData.isContactInDevice() && contactData.isSpammer() && !contactData.isUnsearchableNumber() && !PhoneStateManager.get().isAnyCallActive() && UserCorrectedInfoUtil.a(phone) == null) {
    ...
    protected int getBackgroundColor(Context context) {
        return b.c(CallAppApplication.get(), 2131099675);
    }
    ...
    public void onCallStateChanged(CallData callData) {
        if (callData.getState().isPostCall()) {
            ...
            if (contact == null || !contact.hasPhone(callData.getNumber()) || CallLogUtils.b(contact.getPhone().getRawNumber())) {
                collapse(false);
            } else {
                expandIsSpamView(callData.getNumber(), contact);
            }
        } else if (!callData.getState().isPreCall()) {
            collapse(false);
        }
    }
    ...
    protected void onNegativeBtnClicked(final Phone phone) {
        AnalyticsManager.get().a(Constants.USER_CORRECTED_INFO, ANALYTICS_SPAM_PRESENTER_SHOWN_LABEL, "Negative");
        collapse(true);
        new Task() { // from class: com.callapp.contacts.activity.contact.details.presenter.IsSpamPresenter.2
            @Override // com.callapp.contacts.manager.task.Task
            public void doTask() {
                UserCorrectedInfoUtil.a(IsSpamPresenter.this.presentersContainer.getContact(), phone, false);
    ...
    public void onPositiveBtnClicked(final Phone phone) {
        ...
        AnalyticsManager.get().a(Constants.USER_CORRECTED_INFO, ANALYTICS_SPAM_PRESENTER_SHOWN_LABEL, "Positive", 0.0d, new String[0]);
        new Task() { // from class: com.callapp.contacts.activity.contact.details.presenter.IsSpamPresenter.1
            @Override // com.callapp.contacts.manager.task.Task
            public void doTask() {
                UserCorrectedInfoUtil.a(IsSpamPresenter.this.presentersContainer.getContact(), phone, true);
                EventBusManager.f14368a.a((EventType<L, EventType<InvalidateDataListener, EventBusManager.CallAppDataType>>) InvalidateDataListener.f12949b, (EventType<InvalidateDataListener, EventBusManager.CallAppDataType>) EventBusManager.CallAppDataType.CONTACTS, false);
            }
        }.execute();
        BlockManager.b(this.presentersContainer.getRealContext(), this.presentersContainer.getContact().getNameOrNumber(), phone);
    }
}

...
public class MessageReflection {
    ...
    public static int getSerializedSize(Message message, Map<Descriptors.FieldDescriptor, Object> map) {
        ...
        for (Map.Entry<Descriptors.FieldDescriptor, Object> entry : map.entrySet()) {
            Descriptors.FieldDescriptor key = entry.getKey();
            Object value = entry.getValue();
            i += (!messageSetWireFormat || !key.isExtension() || key.getType() != Descriptors.FieldDescriptor.Type.MESSAGE || key.isRepeated()) ? FieldSet.computeFieldSize(key, value) : CodedOutputStream.computeMessageSetExtensionSize(key.getNumber(), (Message) value);
        }

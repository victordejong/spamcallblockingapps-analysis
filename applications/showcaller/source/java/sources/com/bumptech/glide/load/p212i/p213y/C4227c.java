package com.bumptech.glide.load.p212i.p213y;

import android.content.Context;
import android.net.Uri;
import com.bumptech.glide.load.C4032e;
import com.bumptech.glide.load.data.p208o.C4026b;
import com.bumptech.glide.load.data.p208o.C4027c;
import com.bumptech.glide.load.p212i.AbstractC4191n;
import com.bumptech.glide.load.p212i.AbstractC4193o;
import com.bumptech.glide.load.p212i.C4199r;
import com.bumptech.glide.load.resource.bitmap.VideoDecoder;
import com.bumptech.glide.p222o.C4367d;
import java.io.InputStream;
/* renamed from: com.bumptech.glide.load.i.y.c */
/* loaded from: classes-dex2jar.jar:com/bumptech/glide/load/i/y/c.class */
public class C4227c implements AbstractC4191n<Uri, InputStream> {

    /* renamed from: a */
    private final Context f13107a;

    /* renamed from: com.bumptech.glide.load.i.y.c$a */
    /* loaded from: classes-dex2jar.jar:com/bumptech/glide/load/i/y/c$a.class */
    public static class C4228a implements AbstractC4193o<Uri, InputStream> {

        /* renamed from: a */
        private final Context f13108a;

        public C4228a(Context context) {
            this.f13108a = context;
        }

        @Override // com.bumptech.glide.load.p212i.AbstractC4193o
        /* renamed from: b */
        public AbstractC4191n<Uri, InputStream> mo23078b(C4199r c4199r) {
            return new C4227c(this.f13108a);
        }
    }

    public C4227c(Context context) {
        this.f13107a = context.getApplicationContext();
    }

    /* renamed from: e */
    private boolean m23093e(C4032e c4032e) {
        Long l = (Long) c4032e.m23457c(VideoDecoder.f13230a);
        return l != null && l.longValue() == -1;
    }

    /* renamed from: c */
    public AbstractC4191n.C4192a<InputStream> mo23081b(Uri uri, int i, int i2, C4032e c4032e) {
        if (!C4026b.m23468d(i, i2) || !m23093e(c4032e)) {
            return null;
        }
        return new AbstractC4191n.C4192a<>(new C4367d(uri), C4027c.m23464g(this.f13107a, uri));
    }

    /* renamed from: d */
    public boolean mo23082a(Uri uri) {
        return C4026b.m23469c(uri);
    }
}

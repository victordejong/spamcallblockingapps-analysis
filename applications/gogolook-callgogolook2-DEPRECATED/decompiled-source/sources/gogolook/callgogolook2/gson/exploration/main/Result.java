package gogolook.callgogolook2.gson.exploration.main;

import java.util.ArrayList;
import java.util.List;
import p081h.p203i.p384e.p385v.AbstractC10118a;
import p081h.p203i.p384e.p385v.AbstractC10120c;
/* loaded from: classes2-dex2jar.jar:gogolook/callgogolook2/gson/exploration/main/Result.class */
public class Result {
    @AbstractC10120c("default")
    @AbstractC10118a
    public List<Exploration> _default = new ArrayList();
    @AbstractC10120c("exploration")
    @AbstractC10118a
    public List<Exploration> exploration = new ArrayList();
    @AbstractC10120c("image_prefix")
    @AbstractC10118a
    public String imagePrefix;
}

...
/* loaded from: classes3-dex2jar.jar:com/huawei/hms/framework/common/hianalytics/HianalyticsHelper.class */
public class HianalyticsHelper {
    ...
    private HianalyticsHelper() {
        try {
            ...
            Logger.m38044i(TAG, "Hianalytics sdk not found");
            this.hasHianalytics = false;
        }
        if (!this.hasHianalytics) {
            tryHMSBIInit(ContextHolder.getAppContext());
        }
        Logger.m38041v(TAG, "this time the ha %s, mini %s", Boolean.valueOf(this.hasHianalytics), Boolean.valueOf(this.hasHMSBI));
    }
    ...
    private void onNewEvent(Context context, String str, Map map, int i) {
        ...
        Logger.m38041v(TAG, "data = %s", map);
        try {
            HiAnalyticsUtils.getInstance().onNewEvent(context, str, map, i);
        } catch (NoSuchMethodError e) {
            Logger.m38040w(TAG, "may be you need upgrade stats sdk");
        } catch (Throwable th) {
            Logger.m38044i(TAG, "the stats has other error,pls check it");
        }
    }

    private void tryHMSBIInit(Context context) {
        if (context == null) {
            Logger.m38044i(TAG, "the appContext hasn't init");
            return;
        }
        try {
            ...
            Logger.m38040w(TAG, "maybe you need add base sdk!");
        } catch (Throwable th) {
            Logger.m38040w(TAG, "the hms base has other error!");
        }
    }
    ...
    public boolean isEnableReportNoSeed(Context context) {
        ...
        if (!this.hasHianalytics) {
            Logger.m38044i(TAG, "Hianalytics sdk need to be initialized");
            return false;
        } else if (context == null) {
            Logger.m38044i(TAG, "HianalyticsHelper context can't be null");
            return false;
        } else {
            if (!this.isEnablePrivacyPolicy && Settings.Secure.getInt(context.getContentResolver(), USER_EXPERIENCE_INVOLVED, -1) != 1) {
                Logger.m38044i(TAG, "user experience involved needs to be opened");
                return false;
            }
    ...
    public void onEvent(LinkedHashMap<String, String> linkedHashMap, String str, int i) {
        ...
        if (this.hasHMSBI) {
            ...
            Logger.m38041v(TAG, "the base sdk isn't exsit, and reportType is %s", Integer.valueOf(i));
            return;
        }
        if (this.hasHianalytics && linkedHashMap != null) {
            Logger.m38041v(TAG, "data = %s", linkedHashMap);
            ...
            if (hiAnalyticsInstance2 != null) {
                ...
                Logger.m38047e(TAG, "the has has error,has init but is null!");
            }
        }
    }
    ...
    public void reportException(final Throwable th, final String str) {
        ...
        try {
            ...
            Logger.m38044i(TAG, "reportException error RejectedExecutionException");
        } catch (Exception e2) {
            Logger.m38043i(TAG, "reportException error!", th);
        }
    }

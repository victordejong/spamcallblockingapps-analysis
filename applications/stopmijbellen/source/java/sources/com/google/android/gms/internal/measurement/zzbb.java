package com.google.android.gms.internal.measurement;

import java.util.Arrays;
import java.util.Collections;
import java.util.Iterator;
import java.util.List;
/* loaded from: classes-dex2jar.jar:com/google/android/gms/internal/measurement/zzbb.class */
public final class zzbb {
    /* JADX WARN: Can't fix incorrect switch cases order, some code will duplicate */
    /* JADX WARN: Type inference failed for: r0v118, types: [double] */
    /* JADX WARN: Type inference failed for: r0v126, types: [double] */
    /* JADX WARN: Type inference failed for: r0v155, types: [double] */
    /* JADX WARN: Type inference failed for: r0v185, types: [double] */
    /* JADX WARN: Type inference failed for: r0v189, types: [double] */
    /* JADX WARN: Type inference failed for: r0v193, types: [double] */
    /* JADX WARN: Type inference failed for: r0v264, types: [double] */
    /* JADX WARN: Type inference failed for: r0v293, types: [double] */
    /* JADX WARN: Type inference failed for: r0v296, types: [double] */
    /* JADX WARN: Type inference failed for: r0v298, types: [double] */
    /* JADX WARN: Type inference failed for: r0v301, types: [double] */
    public static zzap zza(String str, zzae zzaeVar, zzg zzgVar, List<zzap> list) {
        boolean z;
        zzae zzaeVar2;
        zzap zzapVar;
        zzah zzahVar;
        zzat zzatVar;
        String str2;
        zzah zzahVar2;
        zzap zzapVar2;
        zzap zzapVar3;
        zzae zzaeVar3;
        zzap zzapVar4;
        zzai zzaiVar;
        zzae zzaeVar4;
        int i;
        switch (str.hashCode()) {
            case -1776922004:
                if (str.equals("toString")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case -1354795244:
                if (str.equals("concat")) {
                    z = false;
                    break;
                }
                z = true;
                break;
            case -1274492040:
                if (str.equals("filter")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case -934873754:
                if (str.equals("reduce")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case -895859076:
                if (str.equals("splice")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case -678635926:
                if (str.equals("forEach")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case -467511597:
                if (str.equals("lastIndexOf")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case -277637751:
                if (str.equals("unshift")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 107868:
                if (str.equals("map")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 111185:
                if (str.equals("pop")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 3267882:
                if (str.equals("join")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 3452698:
                if (str.equals("push")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 3536116:
                if (str.equals("some")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 3536286:
                if (str.equals("sort")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 96891675:
                if (str.equals("every")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 109407362:
                if (str.equals("shift")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 109526418:
                if (str.equals("slice")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 965561430:
                if (str.equals("reduceRight")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 1099846370:
                if (str.equals("reverse")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            case 1943291465:
                if (str.equals("indexOf")) {
                    z = true;
                    break;
                }
                z = true;
                break;
            default:
                z = true;
                break;
        }
        char c = 0;
        switch (z) {
            case false:
                zzap zzt = zzaeVar.zzt();
                if (!list.isEmpty()) {
                    for (zzap zzapVar5 : list) {
                        zzap zza = zzgVar.zza(zzapVar5);
                        if (zza instanceof zzag) {
                            throw new IllegalStateException("Failed evaluation of arguments");
                        }
                        zzae zzaeVar5 = (zzae) zzt;
                        int zzh = zzaeVar5.zzh();
                        if (zza instanceof zzae) {
                            zzae zzaeVar6 = (zzae) zza;
                            Iterator<Integer> zzg = zzaeVar6.zzg();
                            while (zzg.hasNext()) {
                                Integer next = zzg.next();
                                zzaeVar5.zzn(next.intValue() + zzh, zzaeVar6.zzl(next.intValue()));
                            }
                        } else {
                            zzaeVar5.zzn(zzh, zza);
                        }
                    }
                }
                return zzt;
            case true:
                zzh.zza("every", 1, list);
                zzap zza2 = zzgVar.zza(list.get(0));
                if (!(zza2 instanceof zzao)) {
                    throw new IllegalArgumentException("Callback should be a method");
                }
                return zzaeVar.zzh() == 0 ? zzap.zzk : zzc(zzaeVar, zzgVar, (zzao) zza2, Boolean.FALSE, Boolean.TRUE).zzh() != zzaeVar.zzh() ? zzap.zzl : zzap.zzk;
            case true:
                zzh.zza("filter", 1, list);
                zzap zza3 = zzgVar.zza(list.get(0));
                if (!(zza3 instanceof zzao)) {
                    throw new IllegalArgumentException("Callback should be a method");
                }
                if (zzaeVar.zzi() == 0) {
                    zzaeVar2 = new zzae();
                } else {
                    zzap zzt2 = zzaeVar.zzt();
                    zzae zzc = zzc(zzaeVar, zzgVar, (zzao) zza3, null, Boolean.TRUE);
                    zzaeVar2 = new zzae();
                    Iterator<Integer> zzg2 = zzc.zzg();
                    while (zzg2.hasNext()) {
                        zzaeVar2.zzn(zzaeVar2.zzh(), ((zzae) zzt2).zzl(zzg2.next().intValue()));
                    }
                }
                return zzaeVar2;
            case true:
                zzh.zza("forEach", 1, list);
                zzap zza4 = zzgVar.zza(list.get(0));
                if (!(zza4 instanceof zzao)) {
                    throw new IllegalArgumentException("Callback should be a method");
                }
                if (zzaeVar.zzi() == 0) {
                    zzapVar = zzap.zzf;
                } else {
                    zzc(zzaeVar, zzgVar, (zzao) zza4, null, null);
                    zzapVar = zzap.zzf;
                }
                return zzapVar;
            case true:
                zzh.zzc("indexOf", 2, list);
                zzap zzapVar6 = zzap.zzf;
                if (!list.isEmpty()) {
                    zzapVar6 = zzgVar.zza(list.get(0));
                }
                if (list.size() > 1) {
                    c = zzh.zzi(zzgVar.zza(list.get(1)).zzd().doubleValue());
                    if (c >= zzaeVar.zzh()) {
                        zzahVar = new zzah(Double.valueOf(-1.0d));
                        return zzahVar;
                    } else if (c < 0.0d) {
                        c = zzaeVar.zzh() + c;
                    }
                }
                Iterator<Integer> zzg3 = zzaeVar.zzg();
                while (true) {
                    if (zzg3.hasNext()) {
                        int intValue = zzg3.next().intValue();
                        double d = intValue;
                        if (d >= c && zzh.zzf(zzaeVar.zzl(intValue), zzapVar6)) {
                            zzahVar = new zzah(Double.valueOf(d));
                        }
                    } else {
                        zzahVar = new zzah(Double.valueOf(-1.0d));
                    }
                }
                return zzahVar;
            case true:
                zzh.zzc("join", 1, list);
                if (zzaeVar.zzh() == 0) {
                    zzatVar = zzap.zzm;
                } else {
                    if (list.size() > 0) {
                        zzap zza5 = zzgVar.zza(list.get(0));
                        str2 = ((zza5 instanceof zzan) || (zza5 instanceof zzau)) ? "" : zza5.zzc();
                    } else {
                        str2 = ",";
                    }
                    zzatVar = new zzat(zzaeVar.zzs(str2));
                }
                return zzatVar;
            case true:
                zzh.zzc("lastIndexOf", 2, list);
                zzap zzapVar7 = zzap.zzf;
                if (!list.isEmpty()) {
                    zzapVar7 = zzgVar.zza(list.get(0));
                }
                char zzh2 = zzaeVar.zzh() - 1;
                if (list.size() > 1) {
                    zzap zza6 = zzgVar.zza(list.get(1));
                    char zzh3 = Double.isNaN(zza6.zzd().doubleValue()) ? zzaeVar.zzh() - 1 : zzh.zzi(zza6.zzd().doubleValue());
                    zzh2 = zzh3;
                    if (zzh3 < 0.0d) {
                        zzh2 = zzh3 + zzaeVar.zzh();
                    }
                }
                if (zzh2 < 0.0d) {
                    zzahVar2 = new zzah(Double.valueOf(-1.0d));
                } else {
                    int min = (int) Math.min(zzaeVar.zzh(), zzh2);
                    while (true) {
                        if (min < 0) {
                            zzahVar2 = new zzah(Double.valueOf(-1.0d));
                        } else if (!zzaeVar.zzo(min) || !zzh.zzf(zzaeVar.zzl(min), zzapVar7)) {
                            min--;
                        } else {
                            zzahVar2 = new zzah(Double.valueOf(min));
                        }
                    }
                }
                return zzahVar2;
            case true:
                zzh.zza("map", 1, list);
                zzap zza7 = zzgVar.zza(list.get(0));
                if (!(zza7 instanceof zzao)) {
                    throw new IllegalArgumentException("Callback should be a method");
                }
                return zzaeVar.zzh() == 0 ? new zzae() : zzc(zzaeVar, zzgVar, (zzao) zza7, null, null);
            case true:
                zzh.zza("pop", 0, list);
                int zzh4 = zzaeVar.zzh();
                if (zzh4 == 0) {
                    zzapVar2 = zzap.zzf;
                } else {
                    int i2 = zzh4 - 1;
                    zzapVar2 = zzaeVar.zzl(i2);
                    zzaeVar.zzr(i2);
                }
                return zzapVar2;
            case true:
                if (!list.isEmpty()) {
                    for (zzap zzapVar8 : list) {
                        zzaeVar.zzn(zzaeVar.zzh(), zzgVar.zza(zzapVar8));
                    }
                }
                return new zzah(Double.valueOf(zzaeVar.zzh()));
            case true:
                return zzb(zzaeVar, zzgVar, list, true);
            case true:
                return zzb(zzaeVar, zzgVar, list, false);
            case true:
                zzh.zza("reverse", 0, list);
                int zzh5 = zzaeVar.zzh();
                if (zzh5 != 0) {
                    for (int i3 = 0; i3 < zzh5 / 2; i3++) {
                        if (zzaeVar.zzo(i3)) {
                            zzap zzl = zzaeVar.zzl(i3);
                            zzaeVar.zzn(i3, null);
                            int i4 = (zzh5 - 1) - i3;
                            if (zzaeVar.zzo(i4)) {
                                zzaeVar.zzn(i3, zzaeVar.zzl(i4));
                            }
                            zzaeVar.zzn(i4, zzl);
                        }
                    }
                }
                return zzaeVar;
            case true:
                zzh.zza("shift", 0, list);
                if (zzaeVar.zzh() == 0) {
                    zzapVar3 = zzap.zzf;
                } else {
                    zzapVar3 = zzaeVar.zzl(0);
                    zzaeVar.zzr(0);
                }
                return zzapVar3;
            case true:
                zzh.zzc("slice", 2, list);
                if (list.isEmpty()) {
                    zzaeVar3 = zzaeVar.zzt();
                } else {
                    ?? zzh6 = zzaeVar.zzh();
                    double zzi = zzh.zzi(zzgVar.zza(list.get(0)).zzd().doubleValue());
                    char max = zzi < 0.0d ? Math.max(zzi + zzh6, 0.0d) : Math.min(zzi, (double) zzh6);
                    char c2 = zzh6;
                    if (list.size() == 2) {
                        double zzi2 = zzh.zzi(zzgVar.zza(list.get(1)).zzd().doubleValue());
                        c2 = zzi2 < 0.0d ? Math.max(zzh6 + zzi2, 0.0d) : Math.min((double) zzh6, zzi2);
                    }
                    zzae zzaeVar7 = new zzae();
                    int i5 = max;
                    while (true) {
                        zzaeVar3 = zzaeVar7;
                        if (i5 < c2) {
                            zzaeVar7.zzn(zzaeVar7.zzh(), zzaeVar.zzl(i5));
                            i5++;
                        }
                    }
                }
                return zzaeVar3;
            case true:
                zzh.zza("some", 1, list);
                zzap zza8 = zzgVar.zza(list.get(0));
                if (!(zza8 instanceof zzai)) {
                    throw new IllegalArgumentException("Callback should be a method");
                }
                if (zzaeVar.zzh() == 0) {
                    zzapVar4 = zzap.zzl;
                } else {
                    zzai zzaiVar2 = (zzai) zza8;
                    Iterator<Integer> zzg4 = zzaeVar.zzg();
                    while (true) {
                        if (zzg4.hasNext()) {
                            int intValue2 = zzg4.next().intValue();
                            if (zzaeVar.zzo(intValue2) && zzaiVar2.zza(zzgVar, Arrays.asList(zzaeVar.zzl(intValue2), new zzah(Double.valueOf(intValue2)), zzaeVar)).zze().booleanValue()) {
                                zzapVar4 = zzap.zzk;
                            }
                        } else {
                            zzapVar4 = zzap.zzl;
                        }
                    }
                }
                return zzapVar4;
            case true:
                zzh.zzc("sort", 1, list);
                if (zzaeVar.zzh() >= 2) {
                    List<zzap> zzb = zzaeVar.zzb();
                    if (!list.isEmpty()) {
                        zzap zza9 = zzgVar.zza(list.get(0));
                        if (!(zza9 instanceof zzai)) {
                            throw new IllegalArgumentException("Comparator should be a method");
                        }
                        zzaiVar = (zzai) zza9;
                    } else {
                        zzaiVar = null;
                    }
                    Collections.sort(zzb, new zzba(zzaiVar, zzgVar));
                    zzaeVar.zzp();
                    int i6 = 0;
                    for (zzap zzapVar9 : zzb) {
                        zzaeVar.zzn(i6, zzapVar9);
                        i6++;
                    }
                }
                return zzaeVar;
            case true:
                if (list.isEmpty()) {
                    zzaeVar4 = new zzae();
                } else {
                    int zzi3 = (int) zzh.zzi(zzgVar.zza(list.get(0)).zzd().doubleValue());
                    if (zzi3 < 0) {
                        i = Math.max(0, zzaeVar.zzh() + zzi3);
                    } else {
                        i = zzi3;
                        if (zzi3 > zzaeVar.zzh()) {
                            i = zzaeVar.zzh();
                        }
                    }
                    int zzh7 = zzaeVar.zzh();
                    zzaeVar4 = new zzae();
                    if (list.size() > 1) {
                        int max2 = Math.max(0, (int) zzh.zzi(zzgVar.zza(list.get(1)).zzd().doubleValue()));
                        if (max2 > 0) {
                            for (int i7 = i; i7 < Math.min(zzh7, i + max2); i7++) {
                                zzaeVar4.zzn(zzaeVar4.zzh(), zzaeVar.zzl(i));
                                zzaeVar.zzr(i);
                            }
                        }
                        if (list.size() > 2) {
                            for (int i8 = 2; i8 < list.size(); i8++) {
                                zzap zza10 = zzgVar.zza(list.get(i8));
                                if (zza10 instanceof zzag) {
                                    throw new IllegalArgumentException("Failed to parse elements to add");
                                }
                                zzaeVar.zzq((i + i8) - 2, zza10);
                            }
                        }
                    } else {
                        for (int i9 = i; i9 < zzh7; i9++) {
                            zzaeVar4.zzn(zzaeVar4.zzh(), zzaeVar.zzl(i9));
                            zzaeVar.zzn(i9, null);
                        }
                    }
                }
                return zzaeVar4;
            case true:
                zzh.zza("toString", 0, list);
                return new zzat(zzaeVar.zzs(","));
            case true:
                if (!list.isEmpty()) {
                    zzae zzaeVar8 = new zzae();
                    for (zzap zzapVar10 : list) {
                        zzap zza11 = zzgVar.zza(zzapVar10);
                        if (zza11 instanceof zzag) {
                            throw new IllegalStateException("Argument evaluation failed");
                        }
                        zzaeVar8.zzn(zzaeVar8.zzh(), zza11);
                    }
                    int zzh8 = zzaeVar8.zzh();
                    Iterator<Integer> zzg5 = zzaeVar.zzg();
                    while (zzg5.hasNext()) {
                        Integer next2 = zzg5.next();
                        zzaeVar8.zzn(next2.intValue() + zzh8, zzaeVar.zzl(next2.intValue()));
                    }
                    zzaeVar.zzp();
                    Iterator<Integer> zzg6 = zzaeVar8.zzg();
                    while (zzg6.hasNext()) {
                        Integer next3 = zzg6.next();
                        zzaeVar.zzn(next3.intValue(), zzaeVar8.zzl(next3.intValue()));
                    }
                }
                return new zzah(Double.valueOf(zzaeVar.zzh()));
            default:
                throw new IllegalArgumentException("Command not supported");
        }
    }

    private static zzap zzb(zzae zzaeVar, zzg zzgVar, List<zzap> list, boolean z) {
        zzap zzapVar;
        zzh.zzb("reduce", 1, list);
        zzh.zzc("reduce", 2, list);
        zzap zza = zzgVar.zza(list.get(0));
        if (zza instanceof zzai) {
            if (list.size() == 2) {
                zzapVar = zzgVar.zza(list.get(1));
                if (zzapVar instanceof zzag) {
                    throw new IllegalArgumentException("Failed to parse initial value");
                }
            } else if (zzaeVar.zzh() == 0) {
                throw new IllegalStateException("Empty array with no initial value error");
            } else {
                zzapVar = null;
            }
            zzai zzaiVar = (zzai) zza;
            int zzh = zzaeVar.zzh();
            int i = z ? 0 : zzh - 1;
            int i2 = -1;
            int i3 = z ? zzh - 1 : 0;
            if (true == z) {
                i2 = 1;
            }
            int i4 = i3;
            int i5 = i;
            int i6 = i2;
            zzap zzapVar2 = zzapVar;
            if (zzapVar == null) {
                i5 = i + i2;
                i4 = i3;
                i6 = i2;
                zzapVar2 = zzaeVar.zzl(i);
            }
            while ((i4 - i5) * i6 >= 0) {
                int i7 = i4;
                int i8 = i5;
                int i9 = i6;
                zzap zzapVar3 = zzapVar2;
                if (zzaeVar.zzo(i5)) {
                    zzap zza2 = zzaiVar.zza(zzgVar, Arrays.asList(zzapVar2, zzaeVar.zzl(i5), new zzah(Double.valueOf(i5)), zzaeVar));
                    if (zza2 instanceof zzag) {
                        throw new IllegalStateException("Reduce operation failed");
                    }
                    int i10 = i6;
                    i5 += i10;
                    i4 = i4;
                    i6 = i10;
                    zzapVar2 = zza2;
                } else {
                    i5 = i8 + i9;
                    i4 = i7;
                    i6 = i9;
                    zzapVar2 = zzapVar3;
                }
            }
            return zzapVar2;
        }
        throw new IllegalArgumentException("Callback should be a method");
    }

    private static zzae zzc(zzae zzaeVar, zzg zzgVar, zzai zzaiVar, Boolean bool, Boolean bool2) {
        zzae zzaeVar2 = new zzae();
        Iterator<Integer> zzg = zzaeVar.zzg();
        while (zzg.hasNext()) {
            int intValue = zzg.next().intValue();
            if (zzaeVar.zzo(intValue)) {
                zzap zza = zzaiVar.zza(zzgVar, Arrays.asList(zzaeVar.zzl(intValue), new zzah(Double.valueOf(intValue)), zzaeVar));
                if (zza.zze().equals(bool)) {
                    return zzaeVar2;
                }
                if (bool2 == null || zza.zze().equals(bool2)) {
                    zzaeVar2.zzn(intValue, zza);
                }
            }
        }
        return zzaeVar2;
    }
}

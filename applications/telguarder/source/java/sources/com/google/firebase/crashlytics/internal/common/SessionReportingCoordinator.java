...
public class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (!task.isSuccessful()) {
            Logger.getLogger().m1339w("Crashlytics report could not be enqueued to DataTransport", task.getException());
            return false;
        }
        CrashlyticsReportWithSessionId result = task.getResult();
        Logger logger = Logger.getLogger();
        logger.m1348d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
    ...
    private void persistEvent(Throwable th, Thread thread, String str, String str2, long j, boolean z) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m1342v("No log data to include with this event.");
        }
    ...
    public void persistUserId(String str) {
        ...
        if (userId == null) {
            Logger.getLogger().m1342v("Could not persist user ID; no user ID available");
        } else {

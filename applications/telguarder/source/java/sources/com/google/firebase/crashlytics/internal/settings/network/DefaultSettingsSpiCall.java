...
public class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            logger.m1339w("Failed to parse settings JSON from " + this.url, e);
            Logger logger2 = this.logger;
            logger2.m1340w("Settings response " + str);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger = this.logger;
        logger.m1342v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            jSONObject = getJsonObjectFrom(httpResponse.body());
        } else {
            Logger logger2 = this.logger;
            logger2.m1346e("Settings request failed; (status: " + code + ") from " + this.url);
            jSONObject = null;
        }
        return jSONObject;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        if (z) {
            try {
                ...
                this.logger.m1348d("Requesting settings from " + this.url);
                this.logger.m1342v("Settings query params were: " + queryParamsFor);
                jSONObject = handleResponse(applyHeadersTo.execute());
            } catch (IOException e) {
                this.logger.m1345e("Settings request failed.", e);
                jSONObject = null;
            }
            return jSONObject;
        }

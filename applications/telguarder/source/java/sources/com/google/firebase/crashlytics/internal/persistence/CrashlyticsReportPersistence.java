...
public class CrashlyticsReportPersistence {
    ...
    private static void synthesizeNativeReportFile(File file, File file2, CrashlyticsReport.FilesPayload filesPayload, String str) {
        try {
            ...
            logger.m1339w("Could not synthesize final native report file for " + file, e);
        }
    }
    ...
    private void synthesizeReport(File file, long j) {
        ...
        if (filesInDirectory.isEmpty()) {
            Logger.getLogger().m1342v("Session " + file.getName() + " has no events.");
            return;
        }
        ...
        loop0: while (true) {
            ...
            for (File file2 : filesInDirectory) {
                try {
                    ...
                    Logger.getLogger().m1339w("Could not add event to report for " + file2, e);
                }
        ...
        if (arrayList.isEmpty()) {
            Logger.getLogger().m1340w("Could not parse event files for session " + file.getName());
            return;
        }
        ...
        if (file3.isFile()) {
            try {
                ...
                Logger.getLogger().m1339w("Could not read user ID file in " + file.getName(), e2);
                str = null;
            }
        }
    ...
    private static void synthesizeReportFile(File file, File file2, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1339w("Could not synthesize final report file for " + file, e);
        }
    }
    ...
    public void finalizeReports(String str, long j) {
        for (File file : capAndGetOpenSessions(str)) {
            Logger logger = Logger.getLogger();
            logger.m1342v("Finalizing report for session " + file.getName());
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : getAllFinalizedReportFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.m1339w("Could not load report file " + file + "; deleting", e);
                file.delete();
            }
        }
        return arrayList;
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1339w("Could not persist event for session " + str, e);
        }
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().m1348d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1347d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistUserIdForSession(String str, String str2) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1339w("Could not persist user ID for session " + str2, e);
        }
    }
}

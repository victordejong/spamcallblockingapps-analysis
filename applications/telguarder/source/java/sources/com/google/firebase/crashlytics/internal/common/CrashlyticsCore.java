...
/* loaded from: classes2-dex2jar.jar:com/google/firebase/crashlytics/internal/common/CrashlyticsCore.class */
public class CrashlyticsCore {
    ...
    /* loaded from: classes2-dex2jar.jar:com/google/firebase/crashlytics/internal/common/CrashlyticsCore$LogFileDirectoryProvider.class */
    ...
    public Task<Void> doBackgroundInitialization(SettingsDataProvider settingsDataProvider) {
        ...
        try {
            ...
            if (!settingsDataProvider.getSettings().getFeaturesData().collectReports) {
                Logger.getLogger().m1348d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions()) {
                Logger.getLogger().m1340w("Previous sessions could not be finalized.");
            }
            return this.controller.submitAllReports(settingsDataProvider.getAppSettings());
        } catch (Exception e) {
            Logger.getLogger().m1345e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    private void finishInitSynchronously(final SettingsDataProvider settingsDataProvider) {
        ...
        Logger.getLogger().m1348d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            submit.get(4L, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().m1345e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().m1345e("Crashlytics encountered a problem during initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().m1345e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().m1342v("Configured not to require a build ID.");
            return true;
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.3
            ...
            public Boolean call() throws Exception {
                try {
                    ...
                    if (!remove) {
                        Logger.getLogger().m1340w("Initialization marker file was not properly removed.");
                    }
                    return Boolean.valueOf(remove);
                } catch (Exception e) {
                    Logger.getLogger().m1345e("Problem encountered deleting Crashlytics initialization marker.", e);
                    return false;
                }
            }
        });
    ...
    void markInitializationStarted() {
        ...
        Logger.getLogger().m1342v("Initialization marker file was created.");
    }
        ...
        if (isBuildIdValid(appData.buildId, CommonUtils.getBooleanResourceValue(this.context, CRASHLYTICS_REQUIRE_BUILD_ID, true))) {
            try {
                ...
                if (!didPreviousInitializationFail || !CommonUtils.canTryConnection(this.context)) {
                    Logger.getLogger().m1348d("Successfully configured exception handler.");
                    return true;
                }
                Logger.getLogger().m1348d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsDataProvider);
                return false;
            } catch (Exception e) {
                Logger.getLogger().m1345e("Crashlytics was not started due to an exception during initialization", e);

...
public class AsYouTypeFormatter {
    ...
    private void getAvailableFormats(String str) {
        for (Phonemetadata.NumberFormat numberFormat : (!(this.isCompleteNumber && this.extractedNationalPrefix.length() == 0) || this.currentMetadata.getIntlNumberFormatCount() <= 0) ? this.currentMetadata.getNumberFormatList() : this.currentMetadata.getIntlNumberFormatList()) {
    ...
    private void narrowDownPossibleFormats(String str) {
        ...
        while (it.hasNext()) {
            ...
            if (next.getLeadingDigitsPatternCount() != 0) {
                if (!this.regexCache.getPatternForRegex(next.getLeadingDigitsPattern(Math.min(length - 3, next.getLeadingDigitsPatternCount() - 1))).matcher(str).lookingAt()) {
                    it.remove();
                }
            }
        }
    ...
    String attemptToFormatAccruedDigits() {
        for (Phonemetadata.NumberFormat numberFormat : this.possibleFormats) {
            ...
            if (matcher.matches()) {
                ...
                String appendNationalNumber = appendNationalNumber(matcher.replaceAll(numberFormat.getFormat()));

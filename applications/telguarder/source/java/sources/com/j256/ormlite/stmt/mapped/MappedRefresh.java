...
/* loaded from: classes2-dex2jar.jar:com/j256/ormlite/stmt/mapped/MappedRefresh.class */
public class MappedRefresh<T, ID> extends MappedQueryForFieldEq<T, ID> {
    ...
    public static <T, ID> MappedRefresh<T, ID> build(Dao<T, ID> dao, TableInfo<T, ID> tableInfo) throws SQLException {
        ...
        if (idField != null) {
            return new MappedRefresh<>(dao, tableInfo, buildStatement(dao.getConnectionSource().getDatabaseType(), tableInfo, idField), new FieldType[]{tableInfo.getIdField()}, tableInfo.getFieldTypes());
        }
    ...
    public int executeRefresh(DatabaseConnection databaseConnection, T t, ObjectCache objectCache) throws SQLException {
        ...
        for (FieldType fieldType : this.resultsFieldTypes) {
            if (fieldType != this.idField) {
                fieldType.assignField(this.connectionSource, t, fieldType.extractJavaFieldValue(execute), false, objectCache);
            }
        }
        return 1;
    }

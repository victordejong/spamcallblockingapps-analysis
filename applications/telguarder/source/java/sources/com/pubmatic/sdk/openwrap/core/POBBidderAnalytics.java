...
/* loaded from: classes2-dex2jar.jar:com/pubmatic/sdk/openwrap/core/POBBidderAnalytics.class */
public class POBBidderAnalytics {
    ...
    /* renamed from: a */
    private POBRequest f715a;

    /* renamed from: b */
    POBTrackerHandler f716b;

    /* renamed from: c */
    private long f717c = System.currentTimeMillis() / 1000;

    public POBBidderAnalytics(POBRequest pOBRequest, POBTrackerHandler pOBTrackerHandler) {
        this.f715a = pOBRequest;
        this.f716b = pOBTrackerHandler;
    }

    /* renamed from: a */
    private static String m882a(double d) {
        return String.format(Locale.getDefault(), "%.2f", Double.valueOf(d));
    }

    /* renamed from: a */
    private static String m875a(Map<String, String> map) {
    ...
    /* renamed from: a */
    private List<JSONObject> m878a(POBBid pOBBid, Map<String, POBBidderResult<POBBid>> map, POBRequest pOBRequest, Map<String, POBPartnerInfo> map2, JSONObject jSONObject) {
        POBBidderResult<POBBid> pOBBidderResult;
        ArrayList arrayList = new ArrayList();
        for (Map.Entry<String, POBPartnerInfo> entry : map2.entrySet()) {
            POBPartnerInfo pOBPartnerInfo = map2.get(entry.getKey());
            if (pOBPartnerInfo != null && (pOBBidderResult = map.get(pOBPartnerInfo.getPubMaticPartnerId())) != null) {
                    ...
                    if (!bids.isEmpty()) {
                        ...
                        while (i < bids.size()) {
                            ...
                            if (pOBBid2 != null) {
                                ...
                                double parseDouble = Double.parseDouble(m882a(pOBBid2.getPrice()));
                ...
                arrayList.add(m879a(pOBPartnerInfo, pOBBidderResult.getNetworkResult(), jSONObject));
            }
        }
        return arrayList;
    }
    ...
    /* renamed from: a */
    private static Map<String, String> m877a(String str) {
    ...
    /* renamed from: a */
    private static Map<String, String> m876a(String str, POBRequest pOBRequest) {
    ...
    /* renamed from: a */
    private JSONObject m881a(POBAdResponse<POBBid> pOBAdResponse, Map<String, POBPartnerInfo> map, POBRequest pOBRequest, Map<String, POBBidderResult<POBBid>> map2, String str) {
        String str2;
        POBBid pOBBid;
        ...
        jSONObject3.put("ps", new JSONArray(m878a(pOBBid2, map2, pOBRequest, map, new JSONObject()).toArray()));
    ...
    /* renamed from: a */
    private JSONObject m880a(POBAdResponse<POBBid> pOBAdResponse, Map<String, POBPartnerInfo> map, POBRequest pOBRequest, Map<String, String> map2, Map<String, POBBidderResult<POBBid>> map3, String str) {
        if (pOBAdResponse == null || POBUtils.isNullOrEmpty(pOBAdResponse.getLogger())) {
            ...
            return m881a(pOBAdResponse, map, pOBRequest, map3, str);
        }
        ...
        if (jSONArray.length() > 0) {
            ...
            jSONObject2.put("ps", new JSONArray(m878a(pOBAdResponse.getWinningBid(), map3, pOBRequest, map, jSONArray2.length() > 0 ? jSONArray2.getJSONObject(0) : new JSONObject()).toArray()));
    ...
    /* renamed from: a */
    private JSONObject m879a(POBPartnerInfo pOBPartnerInfo, POBNetworkResult pOBNetworkResult, JSONObject jSONObject) {
    ...
    public void executeLogger(POBAdResponse<POBBid> pOBAdResponse, Map<String, POBPartnerInfo> map, Map<String, POBBidderResult<POBBid>> map2, String str) {
        ...
        Map<String, String> m876a = m876a(str2, this.f715a);
        try {
            JSONObject m880a = m880a(pOBAdResponse, map, this.f715a, m876a, map2, str);
            PMLog.debug("POBPartnerTrackerHelper", "Logger JSON - %s", m880a);
            m876a.put("json", String.valueOf(m880a));
            this.f716b.sendTracker(String.format("%s%s", "https://t.pubmatic.com/wl", m875a(m876a)));
    ...
    public void executeTracker(POBAdResponse<POBBid> pOBAdResponse, POBPartnerInfo pOBPartnerInfo) {
        ...
        String adUnitId = this.f715a.getAdUnitId();
        String str2 = adUnitId + "@" + winningBid.getWidth() + "x" + winningBid.getHeight();
        String m882a = m882a(winningBid.getPrice());
        ...
        if (POBUtils.isNullOrEmpty(tracker)) {
            tracker = String.format("https://t.pubmatic.com/wt?bidid=%s&eg=%s&en=%s&iid=%s&kgpv=%s&pdvid=%s&pid=%s&pn=%s&pubid=%s&slot=%s&tst=%s", str, m882a, format, winningBid.getImpressionId(), str2, Integer.valueOf(this.f715a.getVersionId() != null ? this.f715a.getVersionId().intValue() : 0), Integer.valueOf(this.f715a.getProfileId()), bidderCode, this.f715a.getPubId(), str3, Long.valueOf(this.f717c));
        } else {
            try {
                tracker = tracker.replace(URLEncoder.encode("${BID_ID}", StandardCharsets.UTF_8.name()), str).replace(URLEncoder.encode("${PARTNER_NAME}", StandardCharsets.UTF_8.name()), winningBid.getPartnerName()).replace(URLEncoder.encode("${KGPV}", StandardCharsets.UTF_8.name()), str2).replace(URLEncoder.encode("${G_ECPM}", StandardCharsets.UTF_8.name()), m882a).replace(URLEncoder.encode("${N_ECPM}", StandardCharsets.UTF_8.name()), format).replace(URLEncoder.encode("${SLOT_ID}", StandardCharsets.UTF_8.name()), str3);
            } catch (UnsupportedEncodingException e) {
                PMLog.warn("POBPartnerTrackerHelper", "Tracker macro replacement failed.", new Object[0]);
            }
        }
        this.f716b.sendTracker(POBTrackerHandler.sanitizeURLScheme(tracker, Boolean.TRUE));
    }
}

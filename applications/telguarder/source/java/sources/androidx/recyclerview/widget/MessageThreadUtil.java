...
public class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public class C04991 implements ThreadUtil.MainThreadCallback<T> {
        ...
        private Runnable mMainThreadRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.1.1
            ...
            public void run() {
                SyncQueueItem next = C04991.this.mQueue.next();
                while (true) {
                    SyncQueueItem syncQueueItem = next;
                    if (syncQueueItem != null) {
                        int i = syncQueueItem.what;
                        if (i == 1) {
                            C04991.this.val$callback.updateItemCount(syncQueueItem.arg1, syncQueueItem.arg2);
                        } else if (i == 2) {
                            C04991.this.val$callback.addTile(syncQueueItem.arg1, (TileList.Tile) syncQueueItem.data);
                        } else if (i != 3) {
                            Log.e("ThreadUtil", "Unsupported message, what=" + syncQueueItem.what);
                        } else {
                            C04991.this.val$callback.removeTile(syncQueueItem.arg1, syncQueueItem.arg2);
                        }
                        next = C04991.this.mQueue.next();
                    } else {
                        return;
                    }
                }
        ...
        C04991(ThreadUtil.MainThreadCallback mainThreadCallback) {
            MessageThreadUtil.this = r6;
    ...
    public class C05012 implements ThreadUtil.BackgroundCallback<T> {
        ...
        private Runnable mBackgroundRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.2.1
            ...
            public void run() {
                while (true) {
                    SyncQueueItem next = C05012.this.mQueue.next();
                    if (next == null) {
                        C05012.this.mBackgroundRunning.set(false);
                        return;
                    }
                    int i = next.what;
                    if (i == 1) {
                        C05012.this.mQueue.removeMessages(1);
                        C05012.this.val$callback.refresh(next.arg1);
                    } else if (i == 2) {
                        C05012.this.mQueue.removeMessages(2);
                        C05012.this.mQueue.removeMessages(3);
                        C05012.this.val$callback.updateRange(next.arg1, next.arg2, next.arg3, next.arg4, next.arg5);
                    } else if (i == 3) {
                        C05012.this.val$callback.loadTile(next.arg1, next.arg2);
                    } else if (i != 4) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C05012.this.val$callback.recycleTile((TileList.Tile) next.data);
                    }
                }
            }
        };

        C05012(ThreadUtil.BackgroundCallback backgroundCallback) {
            MessageThreadUtil.this = r6;
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new C05012(backgroundCallback);
    }

    @Override // androidx.recyclerview.widget.ThreadUtil
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new C04991(mainThreadCallback);
    }
}

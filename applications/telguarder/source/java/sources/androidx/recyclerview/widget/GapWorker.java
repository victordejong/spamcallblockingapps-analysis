...
public final class GapWorker implements Runnable {
    ...
    private void flushTaskWithDeadline(Task task, long j) {
        RecyclerView.ViewHolder prefetchPositionWithDeadline = prefetchPositionWithDeadline(task.view, task.position, task.immediate ? (char) 65535 : j);
    ...
    public void run() {
        ...
        try {
            ...
            if (!this.mRecyclerViews.isEmpty()) {
                ...
                if (c != 0) {
                    prefetch(TimeUnit.MILLISECONDS.toNanos(c) + this.mFrameIntervalNs);
                }
            }
        } finally {
            this.mPostTimeNs = 0L;

...
public final class SequenceBuilderIterator<T> extends SequenceScope<T> implements Iterator<T>, Continuation<Unit>, KMappedMarker {
    ...
    private final Throwable exceptionalState() {
        ...
        int i = this.state;
        if (i == 4) {
            illegalStateException = new NoSuchElementException();
        } else if (i != 5) {
            illegalStateException = new IllegalStateException("Unexpected state of the iterator: " + this.state);
        } else {
            illegalStateException = new IllegalStateException("Iterator has failed.");
        }
    ...
    public boolean hasNext() {
        while (true) {
            ...
            continuation.resumeWith(Result.m4168constructorimpl(unit));
        }
    }

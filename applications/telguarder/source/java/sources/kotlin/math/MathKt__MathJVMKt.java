...
class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double acosh(double d) {
        ...
        if (d < d2) {
            ...
            c = Math.log(d) + Constants.LN2;
        } else {
            double d3 = d - d2;
            if (d3 >= Constants.taylor_n_bound) {
                c = Math.log(d + Math.sqrt((d * d) - d2));
            } else {
                ?? sqrt = Math.sqrt(d3);
                char c2 = sqrt;
                if (sqrt >= Constants.taylor_2_bound) {
                    c2 = sqrt - (((sqrt * sqrt) * sqrt) / 12);
                }
                c = c2 * Math.sqrt(2.0d);
            }
        }
    ...
    /* renamed from: ln */
    private static final double m382ln(double d) {
        return Math.log(d);
    }

    /* renamed from: ln */
    private static final float m381ln(float f) {
    ...
    public static final double truncate(double d) {
        char c = d;
        if (!Double.isNaN(d)) {
            c = Double.isInfinite(d) ? d : d > ((double) 0) ? Math.floor(d) : Math.ceil(d);
        }
        return c;
    }
    ...
    public static final float truncate(float f) {
        ...
        if (!Float.isNaN(f)) {
            if (Float.isInfinite(f)) {
                ...
                f2 = f > ((float) 0) ? Math.floor(f) : Math.ceil(f);
            }
        }
        return f2;
    }

...
class SamplingEventFilter implements EventFilter {
    ...
    public boolean skipEvent(SessionEvent sessionEvent) {
        boolean z = true;
        boolean z2 = EVENTS_TYPE_TO_SAMPLE.contains(sessionEvent.type) && sessionEvent.sessionEventMetadata.betaDeviceToken == null;
        boolean z3 = Math.abs(sessionEvent.sessionEventMetadata.installationId.hashCode() % this.samplingRate) != 0;
        if (!z2 || !z3) {
            z = false;
        }
        return z;

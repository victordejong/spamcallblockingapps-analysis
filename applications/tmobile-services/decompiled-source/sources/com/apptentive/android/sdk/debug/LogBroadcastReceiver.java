...
public class LogBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        NotificationManagerCompat.m19729d(context).m19731b(intent.getIntExtra("com.apptentive.debug.NOTIFICATION_ID", 0));
        context.sendBroadcast(new Intent("android.intent.action.CLOSE_SYSTEM_DIALOGS"));
        LogMonitor.stopSession(context);
        String action = intent.getAction();
        if ("com.apptentive.debug.ACTION_SEND_LOGS".equals(action)) {
            ApptentiveLog.m15639i("Send Report: User is sending Log Monitor email.", new Object[0]);
            ...
            ApptentiveLog.m15639i("Discard: User exited log monitoring.", new Object[0]);
        } else {
            ApptentiveLog.m15642e("Unexpected action: %s", action);
        }
    }
}

...
public class CrashlyticsController {
    ...
    private static final class PrivacyDialogCheck implements ReportUploader.SendCheck {
        ...
        public boolean canSendReports() {
            ...
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Waiting for user opt-in.");
            create.await();
    ...
    public static final class SendReportRunnable implements Runnable {
        ...
        public void run() {
            if (CommonUtils.canTryConnection(this.context)) {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Attempting to send crash report at time of crash...");
    ...
    private void closeOpenSessions(File[] fileArr, int i, int i2) {
        Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Closing open sessions.");
        while (i < fileArr.length) {
            File file = fileArr[i];
            String sessionIdFromSessionFile = getSessionIdFromSessionFile(file);
            Logger logger = Fabric.getLogger();
            logger.mo288d(CrashlyticsCore.TAG, "Closing session: " + sessionIdFromSessionFile);
    ...
    private void closeWithoutRenamingOrLog(ClsFileOutputStream clsFileOutputStream) {
        if (clsFileOutputStream != null) {
            try {
                ...
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Error closing session file stream in the presence of an exception", e);
            }
        }
    }
    ...
    public void doCloseSessions(SessionSettingsData sessionSettingsData, boolean z) throws Exception {
        ...
        if (listSortedSessionBeginFiles.length <= z) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "No open sessions to be closed.");
            return;
        }
        writeSessionUser(getSessionIdFromSessionFile(listSortedSessionBeginFiles[z ? 1 : 0]));
        if (sessionSettingsData == null) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Unable to close session. Settings are not loaded.");
        } else {
    ...
    public void doOpenSession() throws Exception {
        ...
        Logger logger = Fabric.getLogger();
        logger.mo288d(CrashlyticsCore.TAG, "Opening a new session with ID " + clsuuid);
    ...
    public void doWriteNonFatal(Date date, Thread thread, Throwable th) {
        ...
        if (currentSessionId == null) {
            Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Tried to write a non-fatal exception while no session was open.", null);
            return;
        }
        recordLoggedExceptionAnswersEvent(currentSessionId, th.getClass().getName());
        try {
            try {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Crashlytics is logging non-fatal exception \"" + th + "\" from thread " + thread.getName());
        ...
        try {
            try {
                ...
                clsFileOutputStream = clsFileOutputStream;
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "An error occurred in the non-fatal exception logger", e);
            ...
            Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "An error occurred when trimming non-fatal files.", e5);
            return;
        }
        CommonUtils.closeOrLog(clsFileOutputStream, "Failed to close non-fatal file output stream.");
    }
    ...
    public void finalizeMostRecentNativeCrash(Context context, File file, String str) throws IOException {
        ...
        if (minidumpFromDirectory == null || minidumpFromDirectory.length == 0) {
            Logger logger = Fabric.getLogger();
            logger.mo280w(CrashlyticsCore.TAG, "No minidump data found in directory " + file);
            return;
        }
        ...
        if (!file2.mkdir()) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Couldn't create native sessions directory");
            return;
        }
    ...
    private File[] getTrimmedNonFatalFiles(String str, File[] fileArr, int i) {
        File[] fileArr2 = fileArr;
        if (fileArr.length > i) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, String.format(Locale.US, "Trimming down to %d logged exceptions.", Integer.valueOf(i)));
    ...
    private static void recordFatalExceptionAnswersEvent(String str, String str2) {
        ...
        if (answers == null) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Answers is not available");
        } else {
    ...
    public void recordFatalFirebaseEvent(long j) {
        if (firebaseCrashExists()) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
        } else if (this.firebaseAnalyticsLogger != null) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Logging Crashlytics event to Firebase");
            ...
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Skipping logging Crashlytics event to Firebase, no Firebase Analytics");
        }
    }
    ...
    private void retainSessions(File[] fileArr, Set<String> set) {
        for (File file : fileArr) {
            ...
            if (!matcher.matches()) {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Deleting unknown file: " + name);
                file.delete();
            } else if (!set.contains(matcher.group(1))) {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Trimming session file: " + name);
                file.delete();
            }
        }
    }
    ...
    public void sendSessionReports(SettingsData settingsData) {
        if (settingsData == null) {
            Fabric.getLogger().mo280w(CrashlyticsCore.TAG, "Cannot send reports. Settings are unavailable.");
            return;
        }
    ...
    private void synthesizeSessionFile(File file, String str, File[] fileArr, File file2) {
        ...
        try {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Collecting SessionStart data for session ID " + str);
            ...
            codedOutputStream2 = codedOutputStream;
            clsFileOutputStream = clsFileOutputStream;
            logger.mo285e(CrashlyticsCore.TAG, sb.toString(), e);
    ...
    public void writeFatal(Date date, Thread thread, Throwable th) {
        ...
        if (currentSessionId == null) {
            Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Tried to write a fatal exception while no session was open.", null);
        ...
        try {
            ...
            codedOutputStream2 = codedOutputStream;
            clsFileOutputStream = clsFileOutputStream;
            Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "An error occurred in the fatal exception logger", e);
    ...
    private void writeInitialPartsTo(CodedOutputStream codedOutputStream, String str) throws IOException {
        ...
        for (String str2 : INITIAL_SESSION_PART_TAGS) {
            ...
            if (listFilesMatching.length == 0) {
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Can't find " + str2 + " data for session ID " + str, null);
            } else {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Collecting " + str2 + " data for session ID " + str);
    ...
    private static void writeNonFatalEventsTo(CodedOutputStream codedOutputStream, File[] fileArr, String str) {
        ...
        for (File file : fileArr) {
            try {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, String.format(Locale.US, "Found Non Fatal for session ID %s in %s ", str, file.getName()));
                writeToCosFromFile(codedOutputStream, file);
            } catch (Exception e) {
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Error writting non-fatal to session.", e);
            }
        }
    }
    ...
    private void writeSessionPartFile(String str, String str2, CodedOutputStreamWriteAction codedOutputStreamWriteAction) throws Exception {
        ...
        try {
            File filesDir = getFilesDir();
            ClsFileOutputStream clsFileOutputStream2 = new ClsFileOutputStream(filesDir, str + str2);
            try {
    ...
    private void writeSessionPartsToSessionFile(File file, String str, int i) {
        Logger logger = Fabric.getLogger();
        logger.mo288d(CrashlyticsCore.TAG, "Collecting session parts for ID " + str);
        ...
        Fabric.getLogger().mo288d(CrashlyticsCore.TAG, String.format(Locale.US, "Session %s has fatal exception: %s", str, Boolean.valueOf(z)));
        ...
        Fabric.getLogger().mo288d(CrashlyticsCore.TAG, String.format(Locale.US, "Session %s has non-fatal exceptions: %s", str, Boolean.valueOf(z2)));
        if (z || z2) {
            synthesizeSessionFile(file, str, getTrimmedNonFatalFiles(str, listFilesMatching2, i), z ? listFilesMatching[0] : null);
        } else {
            Logger logger2 = Fabric.getLogger();
            logger2.mo288d(CrashlyticsCore.TAG, "No events present for session ID " + str);
        }
        Logger logger3 = Fabric.getLogger();
        logger3.mo288d(CrashlyticsCore.TAG, "Removing session part files for ID " + str);
    ...
    private void writeSessionUser(String str) throws Exception {
        ...
        writeSessionPartFile(str, SESSION_USER_TAG, new CodedOutputStreamWriteAction() { // from class: com.crashlytics.android.core.CrashlyticsController.25
            @Override // com.crashlytics.android.core.CrashlyticsController.CodedOutputStreamWriteAction
            public void writeTo(CodedOutputStream codedOutputStream) throws Exception {
                SessionProtobufHelper.writeSessionUser(codedOutputStream, userMetaData.f109id, userMetaData.name, userMetaData.email);
            }
        });
    }
    ...
    private static void writeToCosFromFile(CodedOutputStream codedOutputStream, File file) throws IOException {
        ...
        if (!file.exists()) {
            Logger logger = Fabric.getLogger();
            logger.mo285e(CrashlyticsCore.TAG, "Tried to include a file that doesn't exist: " + file.getName(), null);
            return;
        }
        try {
    ...
    void doCleanInvalidTempFiles(File[] fileArr) {
        ...
        for (File file : fileArr) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Found invalid session part file: " + file);
        ...
        if (!hashSet.isEmpty()) {
            ...
            for (File file2 : listFilesMatching(new FilenameFilter() { // from class: com.crashlytics.android.core.CrashlyticsController.15
                ...
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Moving session file: " + file2);
                if (!file2.renameTo(new File(invalidFilesDir, file2.getName()))) {
                    Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Could not move session file. Deleting " + file2);
                    file2.delete();
                }
            }
        ...
        return ((Boolean) this.backgroundWorker.submitAndWait(new Callable<Boolean>() { // from class: com.crashlytics.android.core.CrashlyticsController.13
            ...
            public Boolean call() throws Exception {
                if (CrashlyticsController.this.isHandlingException()) {
                    Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Skipping session finalization because a crash has already occurred.");
                    return Boolean.FALSE;
                }
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Finalizing previously open sessions.");
                CrashlyticsController.this.doCloseSessions(sessionSettingsData, true);
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Closed all previously open sessions");
                return Boolean.TRUE;
            }
        })).booleanValue();
    }
    ...
    void handleUncaughtException(final CrashlyticsUncaughtExceptionHandler.SettingsDataProvider settingsDataProvider, final Thread thread, final Throwable th, final boolean z) {
        synchronized (this) {
            Logger logger = Fabric.getLogger();
            logger.mo288d(CrashlyticsCore.TAG, "Crashlytics is handling uncaught exception \"" + th + "\" from thread " + thread.getName());
    ...
    public void registerAnalyticsEventListener(SettingsData settingsData) {
        if (settingsData.featuresData.firebaseCrashlyticsEnabled) {
            ...
            Logger logger = Fabric.getLogger();
            logger.mo288d(CrashlyticsCore.TAG, "Registered Firebase Analytics event listener for breadcrumbs: " + register);
        }
    }
    ...
    public void submitAllReports(float f, SettingsData settingsData) {
        if (settingsData == null) {
            Fabric.getLogger().mo280w(CrashlyticsCore.TAG, "Could not send reports. Settings are not available.");
            return;
        }

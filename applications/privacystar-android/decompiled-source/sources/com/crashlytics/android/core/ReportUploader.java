...
public class ReportUploader {
    ...
    private class Worker extends BackgroundPriorityRunnable {
        ...
        private void attemptUploadWithRetry() {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Starting report processing in " + this.delay + " second(s)...");
            if (this.delay > 0.0f) {
                try {
            ...
            if (!ReportUploader.this.handlingExceptionCheck.isHandlingException()) {
                if (findReports.isEmpty() || this.sendCheck.canSendReports()) {
                    ...
                    while (!findReports.isEmpty() && !ReportUploader.this.handlingExceptionCheck.isHandlingException()) {
                        Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Attempting to send " + findReports.size() + " report(s)");
                        ...
                        if (!findReports2.isEmpty()) {
                            ...
                            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Report submisson: scheduling delayed retry in " + j + " seconds");
                            try {
                                Thread.sleep(j * 1000);
                                i++;
                                findReports = findReports2;
                ...
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "User declined to send. Removing " + findReports.size() + " Report(s).");
        ...
        public void onRun() {
            try {
                ...
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "An unexpected error occurred while attempting to upload crash reports.", e);
            }
    ...
    List<Report> findReports() {
        ...
        Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Checking for crash reports...");
        ...
        if (completeSessionFiles != null) {
            for (File file : completeSessionFiles) {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Found crash report " + file.getPath());
        ...
        for (String str : hashMap.keySet()) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Found invalid session: " + str);
        ...
        if (linkedList.isEmpty()) {
            Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "No reports found.");
        }
        return linkedList;
    }
    ...
    public boolean forceUpload(Report report) {
        ...
        synchronized (this.fileAccessLock) {
            try {
                ...
                logger.mo284i(CrashlyticsCore.TAG, sb.toString());
                z = false;
                if (invoke) {
                    report.remove();
                    z = true;
                ...
                Fabric.getLogger().mo285e(CrashlyticsCore.TAG, "Error occurred sending report " + report, e);
                z = false;
            }
        }
        return z;
    ...
    public void uploadReports(float f, SendCheck sendCheck) {
        synchronized (this) {
            if (this.uploadThread != null) {
                Fabric.getLogger().mo288d(CrashlyticsCore.TAG, "Report upload has already been started.");
                return;
            }

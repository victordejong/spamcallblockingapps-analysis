    ...
    public ModelLoader.LoadData<Data> buildLoadData(@NonNull Model model, int i, int i2, @NonNull Options options) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            ...
            if (modelLoader.handles(model)) {
                ModelLoader.LoadData<Data> buildLoadData = modelLoader.buildLoadData(model, i, i2, options);
                key = key;
        ...
        if (!arrayList.isEmpty()) {
            loadData = null;
            if (key != null) {
                loadData = new ModelLoader.LoadData<>(key, new MultiFetcher(arrayList, this.exceptionListPool));
            }
        }
        return loadData;
    }

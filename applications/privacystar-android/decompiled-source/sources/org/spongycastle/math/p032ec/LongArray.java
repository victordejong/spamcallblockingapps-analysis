...
/* loaded from: classes3-dex2jar.jar:org/spongycastle/math/ec/LongArray.class */
class LongArray implements Cloneable {
    ...
    private static int bitLength(long j) {
        ...
        if (i4 == 0) {
            int i5 = i3 >>> 8;
            b = i5 == 0 ? bitLengths[i3] : bitLengths[i5] + 8;
        } else {
            int i6 = i4 >>> 8;
            b = i6 == 0 ? bitLengths[i4] + 16 : bitLengths[i6] + 24;
        }
        return i + b;
    }
    ...
    private int degreeFrom(int i) {
        ...
        while (i2 != 0) {
            int i3 = i2 - 1;
            long j = this.m_ints[i3];
            i2 = i3;
            if (j != 0) {
                return (i3 << 6) + bitLength(j);
            }
        }
        return 0;
    }
    ...
    private static long interleave2_32to64(int i) {
        short s = INTERLEAVE2_TABLE[i & 255];
        short s2 = INTERLEAVE2_TABLE[(i >>> 8) & 255];
        return ((((INTERLEAVE2_TABLE[i >>> 24] << 16) | INTERLEAVE2_TABLE[(i >>> 16) & 255]) & BodyPartID.bodyIdMax) << 32) | (BodyPartID.bodyIdMax & (s | (s2 << 16)));
    }
    ...
    */
    private static long interleave2_n(long r8, int r10) {
        /*
        L_0x0000:
            r0 = r10
            r1 = 1
        ...
        L_0x0069:
            ...
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: org.spongycastle.math.p032ec.LongArray.interleave2_n(long, int):long");
    }
    ...
    private static long interleave3_13to65(int i) {
        return ((INTERLEAVE5_TABLE[i >>> 7] & BodyPartID.bodyIdMax) << 35) | (BodyPartID.bodyIdMax & INTERLEAVE5_TABLE[i & 127]);
    }

    private static long interleave3_21to63(int i) {
        int i2 = INTERLEAVE3_TABLE[i & 127];
        return ((INTERLEAVE3_TABLE[i >>> 14] & BodyPartID.bodyIdMax) << 42) | ((INTERLEAVE3_TABLE[(i >>> 7) & 127] & BodyPartID.bodyIdMax) << 21) | (i2 & BodyPartID.bodyIdMax);
    }

    private static long interleave4_16to64(int i) {
        return ((INTERLEAVE4_TABLE[i >>> 8] & BodyPartID.bodyIdMax) << 32) | (BodyPartID.bodyIdMax & INTERLEAVE4_TABLE[i & 255]);
    }
    ...
    private static long interleave7(long j) {
        return (INTERLEAVE7_TABLE[((int) (j >>> 54)) & FrameMetricsAggregator.EVERY_DURATION] << 6) | (Long.MIN_VALUE & j) | INTERLEAVE7_TABLE[((int) j) & FrameMetricsAggregator.EVERY_DURATION] | (INTERLEAVE7_TABLE[((int) (j >>> 9)) & FrameMetricsAggregator.EVERY_DURATION] << 1) | (INTERLEAVE7_TABLE[((int) (j >>> 18)) & FrameMetricsAggregator.EVERY_DURATION] << 2) | (INTERLEAVE7_TABLE[((int) (j >>> 27)) & FrameMetricsAggregator.EVERY_DURATION] << 3) | (INTERLEAVE7_TABLE[((int) (j >>> 36)) & FrameMetricsAggregator.EVERY_DURATION] << 4) | (INTERLEAVE7_TABLE[((int) (j >>> 45)) & FrameMetricsAggregator.EVERY_DURATION] << 5);
    }
    ...
    private static void reduceBitWise(long[] jArr, int i, int i2, int i3, int[] iArr) {
        while (true) {
            ...
            if (i4 >= i3) {
                i2 = i4;
                if (testBit(jArr, i, i4)) {
                    reduceBit(jArr, i, i4, i3, iArr);
                    i2 = i4;
                }
            } else {
                return;
    ...
    private static int reduceInPlace(long[] jArr, int i, int i2, int i3, int[] iArr) {
        ...
        int min2 = (i6 + Math.min(min - max, i3 - i8)) >> 6;
        int i9 = min;
        int i10 = i2;
        if (min2 > 1) {
            int i11 = i2 - min2;
            reduceVectorWise(jArr, i, i2, i11, i3, iArr);
            while (i2 > i11) {
                i2--;
                jArr[i + i2] = 0;
            }
        ...
        if (i9 > i3) {
            reduceBitWise(jArr, i, i9, i3, iArr);
        }
        return i4;
    }
    ...
    public LongArray modInverse(int i, int[] iArr) {
        ...
        if (degree == 0) {
            ...
            while (true) {
                ...
                LongArray longArray5 = longArrayArr[i10];
                int i11 = 1 - i10;
                longArray5.addShiftedByBitsSafe(longArrayArr[i11], iArr2[i11], i8);
                i3 = longArrayArr[i10].degreeFrom(i7);
                if (i3 == 0) {
                    return longArrayArr2[i11];
                }
                int i12 = iArr3[i11];
                longArrayArr2[i10].addShiftedByBitsSafe(longArrayArr2[i11], i12, i8);
                int i13 = i12 + i8;
                if (i13 > i9) {
                    i4 = i13;
                } else {
                    i4 = i9;
                    if (i13 == i9) {
                        i4 = longArrayArr2[i10].degreeFrom(i9);
                    }
                }
                i5 = i8 + (i3 - i7);
                i6 = i10;
    ...
    public LongArray modMultiply(LongArray longArray, int i, int[] iArr) {
        ...
        if (i3 == 1) {
            ...
            multiplyWord(j, longArray.m_ints, i4, jArr, 0);
        ...
        iArr2[1] = i6;
        System.arraycopy(longArray.m_ints, 0, jArr2, i6, i4);
        int i7 = i6;
        ...
        for (int i9 = 0; i9 < i3; i9++) {
            ...
            while (true) {
                long j3 = j2 >>> 4;
                addBoth(jArr5, i10, jArr2, iArr2[((int) j2) & 15], jArr3, iArr2[((int) j3) & 15], i6);
                j2 = j3 >>> 4;
                if (j2 == 0) {
                    break;
                }
    ...
    public LongArray modMultiplyAlt(LongArray longArray, int i, int[] iArr) {
        ...
        if (i4 == 1) {
            ...
            multiplyWord(j, longArray.m_ints, i5, jArr, 0);
        ...
        System.arraycopy(longArray.m_ints, 0, jArr2, i4, i5);
        int i12 = i4;
        for (int i13 = 1; i13 < 8; i13++) {
            i12 += i8;
            shiftUp(jArr2, i4, jArr2, i12, i8, i13);
        }
        int i14 = 0;
        while (true) {
            int i15 = 0;
            do {
                long j2 = jArr2[i15] >>> i14;
                int i16 = i4;
                int i17 = 0;
                while (true) {
                    int i18 = ((int) j2) & i7;
                    if (i18 != 0) {
                        add(jArr2, iArr2[i18] + i15, jArr2, i16, i8);
                    }
                    i17++;
                    if (i17 == 8) {
                        break;
        ...
        while (true) {
            ...
            if ((length & 1) == 0) {
                addShiftedUp(jArr2, iArr2[length >>> 1], jArr2, iArr2[length], i6, 16);
            } else {
                distribute(jArr2, iArr2[length], iArr2[length - 1], iArr2[1], i6);
            }
        }
    }
    ...
    public LongArray modMultiplyLD(LongArray longArray, int i, int[] iArr) {
        ...
        for (int i9 = 56; i9 >= 0; i9 -= 8) {
            for (int i10 = 1; i10 < i3; i10 += 2) {
                ...
                addBoth(jArr5, i10 - 1, jArr2, iArr2[i11 & 15], jArr3, iArr2[(i11 >>> 4) & 15], i6);
            }
        ...
        for (int i12 = 56; i12 >= 0; i12 -= 8) {
            for (int i13 = 0; i13 < i3; i13 += 2) {
                ...
                addBoth(jArr5, i13, jArr2, iArr2[i14 & 15], jArr3, iArr2[(i14 >>> 4) & 15], i6);
            }
            if (i12 > 0) {
                shiftUp(jArr5, 0, i5, 8);
            }
    ...
    public LongArray modSquareN(int i, int i2, int[] iArr) {
        ...
        while (true) {
            i3--;
            if (i3 < 0) {
                return new LongArray(jArr, 0, i4);
            }
            squareInPlace(jArr, i4, i2, iArr);
            i4 = reduceInPlace(jArr, 0, jArr.length, i2, iArr);
        }
    }
    ...
    public LongArray multiply(LongArray longArray, int i, int[] iArr) {
        ...
        while (true) {
            length -= i5;
            if (length == 0) {
                return new LongArray(jArr5, 0, i5);
            }
            addShiftedUp(jArr5, length - i5, jArr5, length, i5, 8);
        }
    }
